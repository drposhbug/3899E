...
void backward(double targetDistance, 
    ...
    // After maxSpeed modification
    if(maxSpeed > 0) {
        maxSpeed = -fabs(maxSpeed);
    }
    Brain.Screen.printAt(10, 60, "maxSpeed after mod: %.2f", maxSpeed);

    // Right before the main control loop
    Brain.Screen.printAt(10, 80, "minSpeedVoltage: %.2f", minSpeedVoltage);
    Brain.Screen.printAt(10, 100, "maxSpeedVoltage: %.2f", maxSpeedVoltage);
    wait(2000, msec);  // Keep display for 2 seconds
