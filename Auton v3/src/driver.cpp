...
void driverControl() {
    ...
    while (true) {
        ...
        double targetSpeedLeft = ((targetPowerLeft * scaleFactor) / 100.0) * maxRPM * wheelCircumferenceCM / 60.0;
        double targetSpeedRight = ((targetPowerRight * scaleFactor) / 100.0) * maxRPM * wheelCircumferenceCM / 60.0;
...
*/
 ...
 //  }
        ...
        // Handle R2 Button for Intake Control
...
*/
    ...
    if (Controller.ButtonY.pressing()) {
        if (!wasYPressed) { // Detect the press event
                ...
                //Move from starting to Load
                if (armstat == ArmPosition::Starting) {
                    // Move arm to Alliance position
                    armMotor.setBrake(brakeType::hold);
                    armMotor.spinToPosition(Load, rotationUnits::deg, 50, velocityUnits::pct, false);
                    armstat = ArmPosition::Load;

                } else if (armstat != ArmPosition::Starting && armstat != Load) {
                    // Move arm to Alliance position
                    armMotor.setBrake(brakeType::hold);
                ...
                } else if (armstat == ArmPosition::Load) { // Added condition
                    // Move arm back to Ready position
                    armMotor.setBrake(brakeType::hold);        
