...
void spotTurnMP(double targetHeading, double maxSpeed, double minSpeed, double breakDistanceInDegrees, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling) { 
    ...
    double slipThresholdSpotTurn = 10; // the lower it is below 1, the slower and more controlled it will be
...
*/
  ...
  //  }
    ...
    if (std::abs(averageMotorVoltage) >= std::abs(maxSpeedVoltage)){
        ...
        Brain.Screen.printAt(10, 80, "Motor L1: %.2f", motorVoltageLeft[0]);
        Brain.Screen.printAt(10, 100, "Motor L2: %.2f", motorVoltageLeft[1]);
        Brain.Screen.printAt(10, 120, "Motor L3: %.2f", motorVoltageLeft[2]);
        Brain.Screen.printAt(10, 140, "Motor R1: %.2f", motorVoltageRight[0]);
        Brain.Screen.printAt(10, 160, "Motor R2: %.2f", motorVoltageRight[1]);
        Brain.Screen.printAt(10, 180, "Motor R3: %.2f", motorVoltageRight[2]);
...
tractionControl::tractionControl(double minSpeedVoltage, double maxSpeedVoltage, double slipThreshold)
    ...
    // adjustSpeed method implementation
    double tractionControl::tractionControlSpeed(double motorVoltage, double wheelSpeed, double robotSpeed) { 
 ...
 motorVoltage = std::copysign(std::max(fabs(minSpeedVoltage), std::min(std::fabs(motorVoltage), std::fabs(maxSpeedVoltage))), motorVoltage);                   
