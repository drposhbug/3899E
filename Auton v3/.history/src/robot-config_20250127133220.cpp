...
//vex::bumper armBumper = vex::bumper(Brain.ThreeWirePort.C); 
vex::limit armBumper = vex::limit(Brain.ThreeWirePort.C);
...
const double TRACK_WIDTH = 32.0;  // Distance between left/right encoders in cm
const double ENCODER_OFFSET_X = 5.0;  // X offset of tracking wheels from center (if not centered)
const double ENCODER_OFFSET_Y = 5.3;  // Y offset of tracking wheels from center
