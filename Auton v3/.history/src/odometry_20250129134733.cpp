...
// First create a struct for any parameters you might need
struct OdometryParams {
    // Add any parameters you might need for odometry
    bool isRunning;
    // You could add other params like update frequency, etc.
};

// Modify the task function to accept void* parameter
int odometryTask(void* params) {
    OdometryParams* p = static_cast<OdometryParams*>(params);
    
    while(p->isRunning) {
        updateOdometry();
        vex::wait(10, msec);  // VEX-specific wait
    }
    return 0;
}