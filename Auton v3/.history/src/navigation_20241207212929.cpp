...
if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && accelCompleted == false) {
    ...
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        motorVoltageLeft[i] = antiSlipLeft[i].increaseSpeed(motorVoltageLeft[i]) + (adjustedHeadingCorrection * 2);      // get slip voltage and Adjust for heading correction
        motorVoltageRight[i] = antiSlipRight[i].increaseSpeed(motorVoltageRight[i]) - (adjustedHeadingCorrection * 2);   
