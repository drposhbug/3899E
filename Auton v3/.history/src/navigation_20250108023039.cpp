...
for (int i = 0; i < 3; i++) {   
    ...
    if (leftBrakeMode == brakeType::coast || rightBrakeMode == brakeType::coast) {
        motorVoltageLeft[i] = minSpeedVoltage + (-11); // Add heading correction and user adjustment factor. Min speed added to reduce regen brake.
        motorVoltageRight[i] = minSpeedVoltage - (adjustedHeadingCorrection * decelHeadingScaling * headingDirection);
          Brain.Screen.printAt(10, 20, "Cruise Heading Correction");
    }

    //Apply brake mode from ABS
    if (leftBrakeMode == brakeType::brake || rightBrakeMode == brakeType::brake) {
    leftMotor[i].stop(leftBrakeMode);
