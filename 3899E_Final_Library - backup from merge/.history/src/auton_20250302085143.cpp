...
void doubleDoinkerBlue()
{
    ...
    const double BLUE_HUE_MAX = 230.0;  // Reduced from 240
        ...
        headingOffset = 300; // Changed from 240 for mirroring (180+60)
        
        // Backup to Pick up Mobile Goal
        backwardMP(70, 40, 296, 20); // Changed from 244 (mirrored angle: 360-(244-180) = 296)
        armMotor1.spinToPosition(570, rotationUnits::deg, 100, velocityUnits::pct, false);
        armMotor2.spinToPosition(570, rotationUnits::deg, 100, velocityUnits::pct, false);
        goalPneumatics.set(true);
        intakeMotor.spin(reverse, 100, velocityUnits::pct);
        ...
        // Turn to go into Tower
        leftMP(222, 60, 20); // Changed from rightMP(138) to leftMP(222) (mirrored angle: 360-138 = 222)
        intakeMotor.spin(reverse, 0, velocityUnits::pct);
    
        forwardMP(43.5, 27, 222); // Updated angle
        
        doinkerPneumaticsLeft.set(true); // Switched from right to left
        wait(200, msec);
        pivotLeftMP(248, 13, 20, 100); // Changed from pivotRightMP(112) (mirrored angle: 360-112 = 248)
        
        doinkerPneumaticsRight.set(true); // Switched from left to right
        wait(300, msec);
        backwardMP(110, 56, 248, 21, 1.5, 0.0008, 0.5, 0.35); // Updated angle
        intakeMotor.spinFor(forward, 0.5, rotationUnits::rev, 100, velocityUnits::pct, true);
        armMotor1.spinToPosition(180, rotationUnits::deg, 100, velocityUnits::pct, false);
        armMotor2.spinToPosition(180, rotationUnits::deg, 100, velocityUnits::pct, false);
        doinkerPneumaticsRight.set(false);
        ...
        rightMP(217, 15, 20); // Changed from leftMP(143) (mirrored angle: 360-143 = 217)
    
        forwardMP(45, 20, 217, 20, 0.8); // Updated angle
    
        leftMP(334, 70, 20); // Changed from rightMP(26) (mirrored angle: 360-26 = 334)
    
        forwardMP(91, 40, 334, 23, 0.4); // Updated angle
        intakeMotor.spin(reverse, 0, velocityUnits::pct);
    
        leftMP(62, 35, 20); // Changed from rightMP(298) (mirrored angle: 360-298 = 62)
        intakeMotor.spin(reverse, 100, velocityUnits::pct);
        ...
        leftMP(125, 25, 40); // Changed from rightMP(235) (mirrored angle: 360-235 = 125)
        intakeMotor.spin(reverse, 100, velocityUnits::pct);
    
        doinkerPneumaticsRight.set(false);
        doinkerPneumaticsLeft.set(false);
        rightMP(110, 5, 40); // Changed from leftMP(250) (mirrored angle: 360-250 = 110)
