    ...
    while (true)
    {
        ...
        if (Controller.ButtonUp.pressing())
        {
            ...
            {                                                              // Detect the press event and ensure arm is not moving
                ...
                armMotor1.spin(directionType::fwd, 50, velocityUnits::pct); // Spin downward at 50% speed
                armMotor2.spin(directionType::fwd, 50, velocityUnits::pct); // Spin downward at 50% speed
            }
        ...
        if ((armBumper1.value() == 1 && fabs(armMotor1.velocity(velocityUnits::rpm)) == 0) && (armBumper2.value() == 1 && fabs(armMotor2.velocity(velocityUnits::rpm)) == 0))
        { // Bumper is pressed
            // if (armBumper.value() == 1) {  // Bumper is pressed
