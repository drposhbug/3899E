...
Brain.Screen.printAt(10, 20, "Before turn - X: %.2f, Y: %.2f, H: %.2f", globalX, globalY, globalHeading);
Brain.Screen.printAt(10, 40, "Target - X: %.2f, Y: %.2f", targetX, targetY);
}
                ...
                double maxSpeed)
{
    ...
    updateOdometry();
    Brain.Screen.clearScreen();
Brain.Screen.printAt(10, 20, "After straight - X: %.2f, Y: %.2f, H: %.2f", globalX, globalY, globalHeading);
}