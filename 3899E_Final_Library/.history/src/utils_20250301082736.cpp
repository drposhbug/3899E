...
int intakeStallTask(void *params) {
    ...
    // Overcrank counter
    int overcrankAttempts = 0;
    const int MAX_OVERCRANK_ATTEMPTS = 3;
    ...
    while (p->isRunning) {
        ...
        if (currentVelocity < p->stallThreshold) {
            ...
            if (stallCounter >= REQUIRED_CONSECUTIVE_STALLS) {
                // Check if we should try to overcrank
                if (overcrankAttempts < MAX_OVERCRANK_ATTEMPTS) {
                    // Apply a pulse of higher power to push deeper
                    intakeMotor.spin(reverse, 100, velocityUnits::pct);
                    wait(300, msec);
                    
                    // Reset stall counter and increment attempts
                    stallCounter = 0;
                    overcrankAttempts++;
                } else {
                    // We've tried enough times, do the final reversal
...
void startIntakeStallDetection(int delayMs = 0) {
    ...
    intakeStallParams.stallThreshold = 1.0;     // 1% velocity threshold
    intakeStallParams.reverseRotation = 210;    // 210 degrees of reversal
    intakeStallParams.reverseSpeed = 60;        // 60% speed for reversal
    intakeStallParams.delayMs = delayMs;        // Set delay parameter
