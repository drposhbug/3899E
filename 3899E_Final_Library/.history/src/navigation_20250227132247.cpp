        ...
        */
        ...
        if (!decel == true || decelCompleted == true)
        {
            if (headingError > 0) { // Counter-clockwise turn (positive error)
                ...
                rightMotor[0].spin(backward, -motorVoltageRight[0], voltageUnits::volt);  // Removed negative sign
                leftMotor[1].stop(hold); // Left side stays stationary
                rightMotor[1].spin(backward, -motorVoltageRight[1], voltageUnits::volt);  // Removed negative sign
                leftMotor[2].stop(brake); // Left side stays stationary
                rightMotor[2].spin(backward, -motorVoltageRight[2], voltageUnits::volt);  // Removed negative sign
            } else { // Clockwise turn (negative error)
                // Right side stationary, left side moves
                leftMotor[0].spin(bac, motorVoltageLeft[0], voltageUnits::volt);
