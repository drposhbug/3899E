...
if (currentState == TURNING) {
    ...
    if (fabs(deltaHeadingRad) > 0.001) {
        // Calculate radii adjusted for encoder offsets from center
        double leftRadius = sqrt(pow(TRACK_WIDTH/2.0 - ENCODER_OFFSET_X, 2) + pow(ENCODER_OFFSET_Y, 2));
        double rightRadius = sqrt(pow(TRACK_WIDTH/2.0 + ENCODER_OFFSET_X, 2) + pow(ENCODER_OFFSET_Y, 2));
        double radius = (leftRadius + rightRadius) / 2.0;
