...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed){
    ...
    double headingError = normHeading360(currentHeading - continuousTarget);;
...
while (std::abs(headingError) > 5) {  // Remove crossed180 check
    ...
    headingError = normHeading360(currentHeading - continuousTarget);
...
//break;   
    ...
    Brain.Screen.printAt(10, 80, "Cruise Phase");    // Else condition - specify actions here if the if condition is not met
    for (int i = 0; i < 3; i++) {
        // Example action: Set motor voltage to target voltage directly
        motorVoltageLeft[i] = maxSpeedVoltage;
        motorVoltageRight[i] = maxSpeedVoltage;
