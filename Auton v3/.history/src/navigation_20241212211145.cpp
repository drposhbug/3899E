...
void spotTurnv2(double targetHeading, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    // Get the current heading and normalize it
    double normTargetHeading = normalizeHeading(targetHeading);
    double normCurrentHeading = normalizeHeading(InertialSensor.heading(degrees));
   
    // Calculate the initial error
    double error = normTargetHeading - normCurrentHeading;
    ...
    while (stabilityCounter < stabilityThreshold) {  // Changed loop condition
        ...
        // Function to calculate average RPM
        for (int i = 0; i < 3; i++) {
            totalLeftRPM += leftMotors[i].velocity(velocityUnits::rpm);
            totalRightRPM += rightMotors[i].velocity(velocityUnits::rpm);
        }
        double avgDriveMotorRPM = ((totalLeftRPM + totalRightRPM) / numberDriveMotor); 

        double driveMotorRadianPerSecond = avgDriveMotorRPM * (2 * M_PI / 60.0);

        // Get actual rotation speed from inertial sensor
        double rotationRateDeg = InertialSensor.gyroRate(axisType::zaxis, velocityUnits::dps);
        double rotationRateRad = rotationRateDeg * (M_PI / 180.0);
...
while (std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) {
    ...
    double robotRadiansPerSecond = rotationRateDeg * (M_PI / 180.0);
...
if (std::abs(currentDistanceInDegrees) < (std::abs(targetDistanceInDegrees) - breakDistanceInDegrees) && accelCompleted == false) {
    ...
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        tractionControl tractionControlLeft[i] = tractionControl(leftMotorRadiansPerSecond[i], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage);
        tractionControl tractionControlRight[i] = tractionControl(rightMotorRadiansPerSecond[i], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage);

        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i]);      // get slip voltage 
        motorVoltageRight[i] = tractionControlRight[i].tractionCotrolSpeed(motorVoltageRight[i]);   

    //    Brain.Screen.printAt(10, 20, "Launch Phase");
    }

     if (std::abs(robotRadiansPerSecond) >= maxRadiansPerSecond * (1 - percenSpeedLoss)){
        accelCompleted = true;
    }
...
tractionControl::tractionControl(double motorSpeed, double robotSpeed, double minSpeedVoltage, double maxSpeedVoltage)
    ...
    // adjustSpeed method implementation
    double tractionControl::tractionControlSpeed(double tractionControlledVoltage) { 
