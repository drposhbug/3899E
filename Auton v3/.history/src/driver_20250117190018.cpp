...
if (Controller.ButtonY.pressing()) {
    if (!wasYPressed) { // Detect the press event
            ...
            //Move from Load1 to Load2
                ...
                // Add bumper check after starting movement
                while (!armBumper.pressing()) {
                    task::sleep(20);  // Small delay while waiting for bumper
                }
                // Once bumper is hit, reset position
                armMotor.resetPosition();
                Brain.Screen.printAt(10, 220, "Arm Position Reset!");
                armstat = ArmPosition::Starting;
...
*/
   ...
   // Reset motor encoder
    ...
    armstat = ArmPosition::Starting;
        ...
        if (armBumper.pressing() && armstat == ArmPosition::Starting) {
        armMotor.resetPosition();
    }
