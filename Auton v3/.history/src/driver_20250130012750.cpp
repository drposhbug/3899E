...
if (Controller.ButtonLeft.pressing()) { 
    if (!wasLeftPressed) { // Detect the press event and ensure arm is not moving
        ...
        if (armstat == ArmPosition::Load1 || armstat == ArmPosition::Load2
            ...
            armMotor.spinToPosition(Descore1, rotationUnits::deg, 100, velocityUnits::pct, false);
            armstat = ArmPosition::Descore1;


        } else if (armstat == ArmPosition::Descore) { 
            // Move arm back to Ready position
            armMotor.setBrake(brakeType::hold);
            armMotor.spinToPosition(Descore2, rotationUnits::deg, 100, velocityUnits::pct, false);
            armstat = ArmPosition::Descore2;
