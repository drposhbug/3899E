...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    double headingOffset = InertialSensor.heading(degrees);
    
    // Get starting position with offset conversion
    double startingNormHeading = normHeading(convertHeading(InertialSensor.heading(degrees), headingOffset));
    ...
    double launchVoltage = std::copysign(3, targetDistanceInDegrees);
   ...
   // MotionPhase currentPhase = READY; 
    ...
    // Loop to continuously adjust motor power based on PID control 
while ((std::fabs(currentDistanceInDegrees) <= std::fabs(targetDistanceInDegrees) - 5) && !turnCompleted) {
    // Get current heading and normalize
    normCurrentHeading = normHeading(InertialSensor.heading(degrees));
    
    // Calculate distance turned, handling wrap-around with normHeading
    double diff = normCurrentHeading - startingNormHeading;
    if (diff > 180) diff -= 360;
    if (diff < -180) diff += 360;
    currentDistanceInDegrees = diff;
...
// Get motor RPM with adjustment for each side
    ...
    //currentDistanceInDegrees = currentHeading - startingHeading;      
