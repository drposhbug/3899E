...
void calculatePathToTarget(double currentX, double currentY, 
    ...
    // Calculate absolute heading to target (order is y,x for atan2)
    double absoluteHeading = atan2(targetY - currentY, targetX - currentX) * 180.0 / M_PI;
    
    // Calculate relative heading by subtracting current robot heading
    heading = normHeading(absoluteHeading - InertialSensor.heading());
}
