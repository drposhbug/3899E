...
void spotTurnMP(double targetHeading, double maxSpeed, double minSpeed, double breakDistanceInDegree, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling) { 
    double normTargetHeading = normalizeHeading(targetHeading);
    double normCurrentHeading = normalizeHeading(InertialSensor.heading(degrees));
   
    // Calculate the initial error
    double targetDistanceInDegree = normTargetHeading - normCurrentHeading;

    
    //Convert % Speed input to voltage with max voltage of 12
    // Calculate maximum speed voltage and match its sign with targetDistance.
    double maxSpeedVoltage = std::copysign(maxSpeed * 0.01 * 12, targetDegreeTravelled);
    ...
    bool decelCompleted = false;
    bool accelCompleted = false;
    double currentDegreeTravelled = 0;
    double targetDriveVoltageLeft = 0; 
    double targetDriveVoltageRight = 0;
    double motorVoltageLeft[3] = {minLaunchSpeedVoltage, minLaunchSpeedVoltage, minLaunchSpeedVoltage};  // Initialize all elements to minimum launch speed
    double motorVoltageRight[3] = {minLaunchSpeedVoltage, minLaunchSpeedVoltage, minLaunchSpeedVoltage};  // Initialize all elements to minimum launch speed
...
ABS ABSRight[3] = {ABS(rightMotor[0], passiveEncoderRight, minSpeedVoltage, vex::brakeType::coast),
    ...
    // Loop to continuously adjust motor power based on PID control 
while (std::abs(currentDegreeTravelled) <= std::abs(targetDegreesTravelled) - 2) {

    currentDegreeTravelled = ((passiveEncoderLeft.position(degrees) + passiveEncoderRight.position(degrees)) / 2.0 / 360.0) * encoderWheelCircumferenceCM;    
    avgMotorVoltage = (motorVoltageLeft[0] + motorVoltageLeft[1] + motorVoltageLeft[2] + motorVoltageRight[0] + motorVoltageRight[1] + motorVoltageRight[2])/6;
...
//Launch Phase
if (std::abs(currentDegreeTravelled) < (std::abs(targetDistance) - breakDistanceInDegree) && accelCompleted == false) {

    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        motorVoltageLeft[i] = antiSlipLeft[i].increaseSpeed(motorVoltageLeft[i]);      // get slip voltage and Adjust for heading correction
        motorVoltageRight[i] = antiSlipRight[i].increaseSpeed(motorVoltageRight[i]);   
...
} else if (std::abs(currentDegreeTravelled) < (std::abs(targetDistance) - breakDistanceInDegree) && accelCompleted == true) {
break;  
    Brain.Screen.printAt(10, 20, "Cruise Phase");    // Else condition - specify actions here if the if condition is not met
    for (int i = 0; i < 3; i++) {
        // Example action: Set motor voltage to target voltage directly
...
tractionControl::tractionControl(double motorSpeed, double robotSpeed, double minSpeedVoltage, double maxSpeedVoltage)
    ...
    // adjustSpeed method implementation
    double tractionControl::tractionControlledSpeed(double tractionControlledVoltage) { 
