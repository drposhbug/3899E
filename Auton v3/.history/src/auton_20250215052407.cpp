...
*/
  ...
  void autonRoutine12() {
    ...
    armMotor.spinToPosition(570, rotationUnits::deg, 100, velocityUnits::pct, true);
    backward(-70,40,25,3, 2.1);  // Changed -3 to 3 (relative)
    goalPneumatics.set(true);
    armMotor.spinToPosition(-40, rotationUnits::deg, 100, velocityUnits::pct, true);
    turnOdometry(-125, 95, 20);  // Changed 93 to -93 (relative)
    straightOdometry(57, 35,17,-125,0.3);  // Changed 93 to -93 (relative)
    doinkerPneumatics.set(true);
    wait(3000,msec);


    backward(-47,20,17,-117);  // Changed 95 to -95 (relative)
    turnOdometry(-200, 180, 25);  // Changed 140 to 220 (absolute, 360-140=220)
    intakeMotor.spin(reverse, 100, velocityUnits::pct);
    doinkerPneumatics.set(false);
   // turnOdometry(-200, 195, 20);  // Changed 168 to 192 (absolute, 360-168=192)
    //turnOdometry(125, 195, 25);  // Changed 235 to 125 (absolute, 360-235=125)
    straightOdometry(35, 25,17,-200,0.2);  // Changed 168 to 192 (matching above absolute heading)
    backward(-35,20,17,-200);  // Changed 95 to -95 (relative)
    turnOdometry(-225, 195, 20);
    straightOdometry(35, 25,17,220,0.2);  // Changed 168 to 192 (matching above absolute heading)
