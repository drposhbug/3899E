...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed){
    ...
    double currentHeading = InertialSensor.rotation();                 // Get continuous rotation
    double fullRotationsLocal = floor(currentHeading / 360.0);         // Determine full rotations
    double continuousTarget = fullRotationsLocal * 360.0 + targetHeading;  // Rebase target heading
    double headingError = continuousTarget - currentHeading;          // Raw error
    headingError = normHeading(headingError);                        // Normalize to get shortest path
    double currentDistanceInDegrees = headingError;

    //Convert % Speed input to voltage with max voltage of 12
    // Remove the negative sign since we want CCW to be positive
...
while (std::abs(headingError) > 5) {
    currentHeading = getAdjustedHeading();
    headingError = currentHeading - targetHeading;  // Reversed order
    // Normalize to [-180, 180] for shortest path
    while (headingError > 180) headingError -= 360.0;
    while (headingError < -180) headingError += 360.0;
    headingError = -headingError;  // Flip sign to make CCW positive
...
if (!decel == true || decelCompleted == true)
{
    for (int i = 0; i < 3; i++) {   
        leftMotor[i].spin(forward, motorVoltageLeft[i], voltageUnits::volt);
        rightMotor[i].spin(forward, -motorVoltageRight[i], voltageUnits::volt); // Note the negative sign here
    }
}      
        //Brain.Screen.printAt(10, 20, "Rotation Sensor: %.2f degrees", rotationDegrees);
        //Brain.Screen.printAt(10, 40, "Current Distance: %.4f", currentDistance);
