...
void calculatePathToTarget(double currentX, double currentY, 
    ...
    // Calculate and normalize absolute target heading
    double absoluteTargetHeading = normHeading(atan2(targetY - currentY, targetX - currentX) * 180.0 / M_PI);
    
    // Get and normalize current absolute robot heading
    double currentAbsoluteHeading = normHeading(InertialSensor.heading());
    
    // Calculate relative heading by normalizing the difference of normalized headings
    heading = normHeading(normHeading(absoluteTargetHeading) - normHeading(currentAbsoluteHeading));
    
    // Debug output
    Brain.Screen.printAt(10, 200, "AbsTgt=%.2f AbsCur=%.2f Rel=%.2f", 
                        absoluteTargetHeading, currentAbsoluteHeading, heading);
}
