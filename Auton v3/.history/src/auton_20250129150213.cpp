...
//turnOdometry(90, 80, 15, 100); 
//task::sleep (2000);
//turnOdometry(270, 80, 15, 100); 
    // Initialize parameters
   // OdometryParams odomParams;
   // odomParams.isRunning = true;
    
    // Create the task with parameters
   // vex::task odomTask(odometryTask, &odomParams);

       // Initialize parameters
    ArmTaskParams armParams;
    armParams.isRunning = true;
    armParams.moveRequested = false;

    // Create the task
    vex::task armControl(armTask, &armParams);
    
    // Example usage:
    armParams.targetPosition = ArmPosition::Side;
   armParams.delayMs = 2000;  // 2 second delay
   armParams.moveRequested = true;
straightToPoint (0, 50, 35, 16);
wait(2000, msec); 
turnToPoint (-50, 0, 80);    // Second move - create new task
    armParams.targetPosition = ArmPosition::Ready;
    armParams.delayMs = 1000;
    armParams.moveRequested = true;
    vex::task armControl2(armTask, &armParams);

// Later move to another position
    armParams.targetPosition = ArmPosition::Ready;
    armParams.delayMs = 2000;  // 1 second delay
    armParams.moveRequested = true;
wait(4000, msec); 
