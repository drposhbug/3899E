...
while (std::abs(currentDistance) <= std::abs(targetDistance) - 2) {
    ...
    // Calculate the heading correction using the PID controller 
    double headingCorrection = headingPID.calculate(normTargetHeading, normalizeHeading(InertialSensor.heading()));
    double encoderSpeedRPM = (passiveEncoderLeft.velocity(vex::velocityUnits::rpm) + passiveEncoderRight.velocity(vex::velocityUnits::rpm))/2;
...
if (encoderSpeedRPM < minDriveMotorRPM) {
    decelCompleted = true;
...
//Final Approach Phase 
       ...
       // double distanceError = targetDistance - currentDistance;
            ...
            // Set the speed to minSpeed after the first stabilization
        //    distanceCorrection = (distanceCorrection > 0) ? minSpeedVoltage : -minSpeedVoltage;
        double adjustedHeadingCorrection = headingCorrection * encoderSpeedRPM / maxRPM * 2.0; //dynamically reduce heading correction at slower speed based on percentage of max speed
...
void pidStraightDistanceSlipABS(double targetHeading, double targetDistance, double maxSpeed, double kp_heading, double ki_heading, double kd_heading, double kp_distance, double ki_distance, double kd_distance, double minSpeed, double breakDistance) { 
    ...
    // Declare arrays of Antilock Brake instances for each wheel
    slipControl antiSlipLeft[3] = {slipControl(leftMotor[0], passiveEncoderLeft, minLaunchSpeedVoltage),
                                slipControl(leftMotor[1], passiveEncoderLeft, minLaunchSpeedVoltage),
                                slipControl(leftMotor[2], passiveEncoderLeft, minLaunchSpeedVoltage)};
                                
    slipControl antiSlipRight[3] = {slipControl(rightMotor[0], passiveEncoderRight, minLaunchSpeedVoltage),
                                slipControl(rightMotor[1], passiveEncoderRight, minLaunchSpeedVoltage),
                                slipControl(rightMotor[2], passiveEncoderRight, minLaunchSpeedVoltage)};
...
slipControl::slipControl(vex::motor& motor, vex::rotation& encoder, double minSpeedVoltage)
    : motor(motor), encoder(encoder), minSpeedVoltage(minSpeedVoltage) {} // Initialized slipThreshold
...
//}
 ...
 slipMotorVoltage = std::copysign(std::max(minSpeedVoltage, std::min(std::abs(slipMotorVoltage), std::abs(targetVoltage))), targetVoltage);                      
...
void pidStraightDistanceLaunchABS(double targetHeading, double targetDistance, double maxSpeed, double kp_heading, double ki_heading, double kd_heading, double kp_distance, double ki_distance, double kd_distance, double minSpeed, double breakDistance) { 
...
//Launch Phase
if ( std::abs(avgMotorVoltage) > 0 &&  std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && accelCompleted == false) {
...
} else if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && accelCompleted == false) {
    

    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left sid
        motorVoltageLeft[i] = antiSlipLeft[i].increaseSpeed(motorVoltageLeft[i]) + adjustedHeadingCorrection;      // get slip voltage and Adjust for heading correction
        motorVoltageRight[i] = antiSlipRight[i].increaseSpeed(motorVoltageRight[i]) - adjustedHeadingCorrection;   
...
//If declerating then go to ABS routine
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        motorVoltageLeft[i] = ABSLeft[i].reduceSpeed();
        if (motorVoltageLeft[i] != 0) {
            motorVoltageLeft[i] += adjustedHeadingCorrection; // Add heading correction only if voltage is not zero
        }
        leftMotor[i].setBrake(ABSLeft[i].getBrakeMode());
        */
        ...
        if (motorVoltageLeft[i] != 0) {
            motorVoltageLeft[i] += adjustedHeadingCorrection * .5; // Add heading correction only if voltage is not zero
        }
        ...
        motorVoltageRight[i] = ABSRight[i].reduceSpeed();
        if (motorVoltageRight[i] != 0) {
            motorVoltageRight[i] -= adjustedHeadingCorrection; // Subtract heading correction if voltage is not zero.
        }
        rightMotor[i].setBrake(ABSRight[i].getBrakeMode());  
        */
        ...
        if (motorVoltageRight[i] != 0) {
            motorVoltageRight[i] -= adjustedHeadingCorrection * .5 ; // Subtract heading correction only if voltage is not zero
        }
