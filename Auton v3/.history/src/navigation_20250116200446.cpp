...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    // Get current heading and normalize it
    double startingNormHeading = normHeading(InertialSensor.heading(degrees));
    
    // Calculate target heading by adding turn amount to current heading
    double targetHeading = startingNormHeading + turnDegrees;
    double normTargetHeading = normHeading(targetHeading);
    
    // Initial heading error is our target distance to turn
    double targetDistanceInDegrees = normTargetHeading - startingNormHeading;
