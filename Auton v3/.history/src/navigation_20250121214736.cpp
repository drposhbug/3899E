...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    double launchVoltage = std::copysign(3.5, targetDistanceInDegrees); //3.5 is good
...
if (fabs(currentDistanceInDegrees) < (fabs(targetDistanceInDegrees) - fabs(breakDistanceInDegrees)) && !crossed180 && !accelCompleted && !decel) {
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
  ...
  //  }
    ...
    if (std::fabs(averageMotorVoltage) >= std::fabs(maxSpeedVoltage)){
        ...
        Brain.Screen.printAt(10, 160, "LeftRPM[0]: %.2f", leftMotorRPM[0]);
        Brain.Screen.printAt(10, 180, "LeftEncRPM: %.2f", leftEncoderRPM);
        Brain.Screen.printAt(10, 200, "LeftVolt[0]: %.2f", motorVoltageLeft[0]);
