...
void calculatePathToTarget(double currentX, double currentY, 
    ...
    // Since we should already be roughly facing the target direction,
    // relative heading should be close to 0, just accounting for small corrections
    heading = normHeading(absoluteTargetHeading - absoluteTargetHeading);
