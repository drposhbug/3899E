...
while (std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) {
    ...
    for (int i = 0; i < 3; i++) {
        double leftMotorRadiansPerSecond[i] = leftMotors[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
        double rightMotorRadiansPerSecond[i] = leftMotors[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
    }
