                      ...
                      double maxSpeed)
{
    ...
    while (std::fabs(currentDistance) <= fabs(targetDistance) - 4)
    {
        ...
        */
        ...
        double headingError = normalizeAngle180(targetHeading - currentHeading);
        double headingCorrection = headingPID.calculate(0, headingError); // Error is now the setpoint
...
// Force distance to be negative to ensure backward motion
targetDistance = -fabs(targetDistance);

// Call straightOdometry with the negative distance
straightOdometry(targetDistance,
           breakDistance,
           minSpeed,
           targetHeading,
           kp_heading,
