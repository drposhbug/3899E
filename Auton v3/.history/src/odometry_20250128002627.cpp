...
void calculatePathToTarget(double currentX, double currentY, 
    ...
    // Calculate absolute target heading (order is y,x for atan2)
    double absoluteTargetHeading = atan2(targetY - currentY, targetX - currentX) * 180.0 / M_PI;
    
    // Get current absolute robot heading
    double currentAbsoluteHeading = InertialSensor.heading();
    
    // Calculate relative heading (difference between target and current)
    // This gives us the relative angle where 0 is straight ahead
    heading = normHeading(absoluteTargetHeading - currentAbsoluteHeading);
