...
void skills()
{
    // Make sure the optical sensor is initialized
initializeOpticalSensor();
...
while (!ringDetected && timeoutCounter < 150) {
    // Use the detectColor function from utils.cpp
    ringDetected = detectColor();
    
    if (ringDetected) {
        Brain.Screen.print("Ring Detected!");
        intakeMotor.stop();
    }
...
// If we exit without detecting, still need to continue with forward movement
if (!ringDetected) {
    // Optional: display a message that timeout occurred
    Brain.Screen.print("Detection timeout");
}
