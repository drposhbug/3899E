...
void skills()
{
    ...
    // Define color task parameters
    ColorTaskParams colorTaskParams;
    colorTaskParams.isRunning = true;
    colorTaskParams.targetColor = Color::BLUE; // Set ejection color to BLUE
    colorTaskParams.delayMs = 250;             // Increased delay before stopping intake (from 80 to 250)
    
    // Display test information
    Brain.Screen.clearScreen();
    Brain.Screen.setCursor(1, 1);
    Brain.Screen.print("Ring Ejection Test");
    Brain.Screen.setCursor(2, 1);
    Brain.Screen.print("Target Color: BLUE");
    
    // Start the color detection task
    vex::task colorTask(colorDetectionTask, &colorTaskParams);
    
    // Reset color detection state
    resetColorDetection();
    
    // Start intake motor continuously
    intakeMotor.spin(reverse, 100, velocityUnits::pct);
    
    // Display current hue value for debugging
    while(true) {
        Brain.Screen.setCursor(3, 1);
        Brain.Screen.print("Current Hue: %f     ", opticalSensor.hue());
        Brain.Screen.setCursor(4, 1);
        Brain.Screen.print("Brightness: %f     ", opticalSensor.brightness());
   ...
   wait(300, msec);
    ...
    backwardMP(47, 25, 224, 16, 1.5, 0.0008, 0.5, 0.35);
    ...
    // Reset color detection state to ensure clean detection
resetColorDetection();

// Wait for ring to be detected or timeout
bool ringDetected = false;
int timeoutCounter = 0;
const int maxTimeout = 100; // 100 * 10ms = 1 second timeout

while (!ringDetected && timeoutCounter < maxTimeout) {
    ringDetected = detectColor(); // Uses your existing function
    
    if (ringDetected) {
        // Ring detected - optional: show feedback
        Brain.Screen.clearLine(2);
        Brain.Screen.setCursor(2, 1);
        Brain.Screen.print("Ring Captured!");
        
        // Stop the intake motor to secure the ring
        intakeMotor.stop();
    }
...
/*    
    forwardMP(90, 50, 27, 16, 1.5, 0.03, 0.1);
