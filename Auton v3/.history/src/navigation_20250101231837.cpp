...
// Calculate translational speed (cm/s) from RPM
for (int i = 0; i < 3; i++) {

    // Calculate angular velocity (radians/s) from translational speed
    leftMotorAngularRadians[i] = leftMotorCMPerSecond[i] / (trackWidth / 2.0);
    rightMotorAngularRadians[i] = rightMotorCMPerSecond[i] / (trackWidth / 2.0);
    
    // Calculate total angular velocity
    totalMotorRadiansPerSecond += fabs(leftMotorAngularRadians[i]) + fabs(rightMotorAngularRadians[i]);
}
...
if (std::abs(currentDistanceInDegrees) < (std::abs(targetDistanceInDegrees) - breakDistanceInDegrees) && !turnCompleted && !accelCompleted) {
    ...
    for (int i = 0; i < 3; i++) {   
        //Find the lower of the 2 voltages
        double lowerVoltage = std::min(motorVoltageLeft[i], motorVoltageRight[2-i]);

        //Set left and right to the lowest motor voltage to reduce slip but keep it balanced on both side so the center of the spin is consistent
        motorVoltageRight[i] = lowerVoltage;
        motorVoltageLeft[2-i] = lowerVoltage;
    }
...
tractionControl::tractionControl(double minSpeedVoltage, double maxSpeedVoltage, double slipThreshold)
   ...
   Brain.Screen.printAt(10, 100, "Motor Speed: %.2f, Robot Speed: %.2f", wheelSpeed,robotSpeed); 
    ...
    //Brain.Screen.printAt(10, 20, "Launch Phase");
    if (std::abs(wheelSpeed) > std::abs(robotSpeed) * (slipThreshold)) {                      // Checking for slip plus slip threshold allowance
        motorVoltage = motorVoltage / accelFactor;    
    
    } else if (std::abs(wheelSpeed) <= std::abs(robotSpeed) * (slipThreshold))  {
        motorVoltage = motorVoltage * accelFactor;      
