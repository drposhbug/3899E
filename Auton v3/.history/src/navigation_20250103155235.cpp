...
void spotTurnMP(double targetHeading, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    double startingNormHeading = normHeading(InertialSensor.heading(degrees));
    ...
    //double avgMotorVoltage = 0;
    double launchVoltage = std::copysign(4.5, targetDistanceInDegrees); //3.5 is good
    double minLaunchSpeedVoltage = std::copysign(std::min(abs(maxSpeedVoltage), abs(launchVoltage)), targetDistanceInDegrees);
    double percentSpeedLoss = 0.2;
    double slipThresholdSpotTurn = 1.1; // the lower it is below 1, the slower and more controlled it will be
    double ABSLockThresholdSpotTurn = 0.7;
   ...
   // MotionPhase currentPhase = READY; 
    ...
    // Declare arrays of Slip Control instances for each wheel
    tractionControl tractionControlLeft[3] = {tractionControl(minLaunchSpeedVoltage, maxSpeedVoltage, slipThresholdSpotTurn),
                                              tractionControl(minLaunchSpeedVoltage, maxSpeedVoltage, slipThresholdSpotTurn),
                                              tractionControl(minLaunchSpeedVoltage, maxSpeedVoltage, slipThresholdSpotTurn)};
                                
    tractionControl tractionControlRight[3] = {tractionControl(minLaunchSpeedVoltage, maxSpeedVoltage, slipThresholdSpotTurn),
                                               tractionControl(minLaunchSpeedVoltage, maxSpeedVoltage, slipThresholdSpotTurn),
                                               tractionControl(minLaunchSpeedVoltage, maxSpeedVoltage, slipThresholdSpotTurn)};

    ABSController ABSControllerLeft[3] = {ABSController(minSpeedVoltage, ABSLockThresholdSpotTurn),
                                          ABSController(minSpeedVoltage, ABSLockThresholdSpotTurn),
                                          ABSController(minSpeedVoltage, ABSLockThresholdSpotTurn)};
                            
    ABSController ABSControllerRight[3] = {ABSController(minSpeedVoltage, ABSLockThresholdSpotTurn),
                                          ABSController(minSpeedVoltage, ABSLockThresholdSpotTurn),
                                          ABSController(minSpeedVoltage, ABSLockThresholdSpotTurn)};
...
while ((std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 5) && turnCompleted == false) {
    ...
    for (int i = 0; i < 3; i++) {
       ...
       // totalMotorRadiansPerSecond += std::abs(motorRadiansPerSecondLeft[i]) + std::abs(motorRadiansPerSecondRight[i]);
    }
   ...
   // double avgMotorRadianPerSecond =  totalMotorRadiansPerSecond / numberDriveMotor; // Calculate average motor radians per second
    ...
    //Get current raw degrees
    currentNormHeading = normHeading(InertialSensor.heading(degrees));
...
tractionControl::tractionControl(double minSpeedVoltage, double maxSpeedVoltage, double slipThreshold)
   ...
   Brain.Screen.printAt(10, 100, "Motor Speed: %.2f, Robot Speed: %.2f", wheelSpeed,robotSpeed); 

    //Brain.Screen.printAt(10, 20, "Launch Phase");
    if (std::fabs(wheelSpeed) > std::fabs(robotSpeed) * (slipThreshold)) {                      // Checking for slip plus slip threshold allowance
        motorVoltage = motorVoltage / accelFactor;    
    
    } else if (std::fabs(wheelSpeed) <= std::fabs(robotSpeed) * (slipThreshold))  {
        motorVoltage = motorVoltage * accelFactor;      
...
ABSController::ABSController(double minSpeedVoltage, double ABSLockThreshold)
    : minSpeedVoltage(minSpeedVoltage), ABSLockThreshold(ABSLockThreshold) {}

// adjustSpeed method implementation
ABSReturn ABSController::ABSSpeedReduction(double motorVoltage, double wheelSpeed, double robotSpeed) {
    ABSReturn result;  // Create ABSResult instance to store motor voltage and brake mode
        ...
        if (std::abs(wheelSpeed) >= std::abs(robotSpeed) * ABSLockThreshold) {
            // Wheels are locking
            result.motorVoltage = minSpeedVoltage;  // Set motor voltage to minimum
            result.brakeMode = vex::brakeType::coast;  // Prevent further locking
            //Brain.Screen.printAt(60, 40, "coast (lock)");
...
void straight(double targetHeading, double targetDistance, double maxSpeed, double breakDistance, double kp_heading, double ki_heading, double kd_heading, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling, double minSpeed) { 
    ...
    double slipThreshold = 1.1; // the lower it is below 1, the slower and more controlled it will be
    double ABSLockThreshold = 0.7;
    double accelFactorLaunch = 1.4;
    bool decel = false;
    bool decelCompleted = false;
    bool accelCompleted = false;
...
*/
    ...
    ABSController ABSControllerLeft[3] = {ABSController(minSpeedVoltage, ABSLockThreshold),
                                          ABSController(minSpeedVoltage, ABSLockThreshold),
                                          ABSController(minSpeedVoltage, ABSLockThreshold)};
                            
    ABSController ABSControllerRight[3] = {ABSController(minSpeedVoltage, ABSLockThreshold),
                                          ABSController(minSpeedVoltage, ABSLockThreshold),
                                          ABSController(minSpeedVoltage, ABSLockThreshold)};
...
//Launch Phase
if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && !accelCompleted) {

    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        // Get motor speed in RPM
        leftMotorRPM [i] = leftMotor[i].velocity(vex::velocityUnits::rpm);
        rightMotorRPM [i] = rightMotor[i].velocity(vex::velocityUnits::rpm);

        //Call traction cotrol class and get adjusted motor voltage
        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i], leftMotorRPM[i], encoderAvgRPMScaled, accelFactorLaunch) + (adjustedHeadingCorrection * accelHeadingScaling);      // get slip voltage and Adjust for heading correction
        motorVoltageRight[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], encoderAvgRPMScaled, accelFactorLaunch) - (adjustedHeadingCorrection * accelHeadingScaling);   
    }  

    if (std::abs(avgMotorVoltage) >= std::abs(maxSpeedVoltage)){
        accelCompleted = true;
    }
...
for (int i = 0; i < 3; i++) {   
    ...
    //Left Side
    ABSReturn leftResult = ABSControllerLeft[i].ABSSpeedReduction(motorVoltageLeft[i], leftMotorRPM[i], encoderAvgRPMScaled); // Get ABSResult containing motor voltage and brake mode
    
    //Adding heading PID correction during coast phase and reduce regen braking by adding min speed (smaller the voltage, the greater the resistance).
    if (leftResult.brakeMode == brakeType::coast) {
        motorVoltageLeft[i] += minSpeedVoltage + (adjustedHeadingCorrection * decelHeadingScaling); // Add heading correction and user adjustment factor. Min speed added to reduce regen brake.
    }

    //Apply brake mode from ABS
    leftMotor[i].stop(leftResult.brakeMode);

    //Right Side
    ABSReturn rightResult = ABSControllerRight[i].ABSSpeedReduction(motorVoltageRight[i], rightMotorRPM[i], encoderAvgRPMScaled); // Get ABSResult containing motor voltage and brake mode
    motorVoltageRight[i] = rightResult.motorVoltage; // Extract motor voltage from ABSResult
...
if (abs(avgMotorVoltage) <= abs(minSpeedVoltage)) {    
    decelCompleted = true;
}
