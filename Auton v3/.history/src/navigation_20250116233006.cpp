...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    // Get starting position - use raw values for distance tracking
    double startingHeading = InertialSensor.heading(degrees);
    double currentDistanceInDegrees = 0;
    double targetDistanceInDegrees = turnDegrees;

    Brain.Screen.printAt(10, 40, "Start Head: %.2f", startingHeading);
...
while ((std::fabs(currentDistanceInDegrees) <= std::fabs(targetDistanceInDegrees) - 5) && !turnCompleted) {
        // Use raw values for distance calculation
        double currentHeading = InertialSensor.heading(degrees);
        currentDistanceInDegrees = currentHeading - startingHeading;
        
        // Use normalized heading when needed for other calculations
        double normCurrentHeading = normHeading(currentHeading);
/*
