...
void spotTurnMP(double targetHeading, double maxSpeed, double minSpeed, double breakDistanceInDegrees, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling) { 
    double normTargetHeading = normalizeHeading(targetHeading);
    double startingNormalizedDegrees = normalizeHeading(InertialSensor.heading(degrees));
    double normCurrentHeading = startingNormalizedDegrees;
    ...
    double currentNormalizedDegrees = 0;
...
ABSController ABSControllerRight[3] = {ABSController(motorRadiansPerSecondRight[0], robotRadiansPerSecond, minSpeedVoltage),
    ...
    // Loop to continuously adjust motor power based on PID control 
while ((std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) && (currentNormalizedDegrees * normTargetHeading >= 0)) {
    ...
    double currentNormalizedDegrees = normalizeHeading(InertialSensor.heading(degrees));
    double minSpeedVoltage = std::copysign(minSpeed * 0.01 * 12, targetDistanceInDegrees);

    //Calculate current distance in degrees
    //currentDistanceInDegrees = normalizeHeading(InertialSensor.heading()) - startingDegrees; 
    currentDistanceInDegrees = currentNormalizedDegrees - startingNormalizedDegrees;       
...
if (std::abs(currentDistanceInDegrees) < (std::abs(targetDistanceInDegrees) - breakDistanceInDegrees) && accelCompleted == false) {
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        // Track the lowest motor voltage for the left side
        if (motorVoltageLeft[i] < lowestMotorVoltage) {
            lowestMotorVoltage = motorVoltageLeft[i];
        }    

        if (motorVoltageRight[i] < lowestMotorVoltage) {
            lowestMotorVoltage = motorVoltageRight[i];
    ...
    for (int i = 0; i < 3; i++) {   
        // use the lowest voltage to maintain equal traction on all motors
        motorVoltageLeft[i] = lowestMotorVoltage;
    }
...
turnDirection = std::copysign(turnDirection, targetDistanceInDegrees);
