...
while (std::abs(currentDistance) <= std::abs(targetDistance) - 2) {
    ...
    passiveEncoderLeft = passiveEncoderLeft.velocity(vex::velocityUnits::rpm) * 
    double avgEncoderRPM = ((passiveEncoderLeft.velocity(vex::velocityUnits::rpm) + passiveEncoderRight.velocity(vex::velocityUnits::rpm))/2)  * (wheelCircumferenceCM / encoderWheelCircumferenceCM);
