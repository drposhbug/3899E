...
while ((std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) && turnCompleted == false) {
   ...
   // double avgMotorRadianPerSecond =  totalMotorRadiansPerSecond / numberDriveMotor; // Calculate average motor radians per second
    ...
    double currentNormHeading = normHeading(InertialSensor.heading(degrees));
    //double minSpeedVoltage = std::copysign((minSpeed * 0.01 * 12), normTargetHeading);
 ...
 */   
        Brain.Screen.printAt(10, 20, "Launch Phase");
        Brain.Screen.printAt(10, 180, "Current Heading: %.2f", std::round(currentNormHeading) * normTargetHeading);
...
//turnDirection = std::copysign(turnDirection, normTargetHeading);
    ...
    turnCompleted = std::round(currentNormHeading) * normTargetHeading >= 0;
