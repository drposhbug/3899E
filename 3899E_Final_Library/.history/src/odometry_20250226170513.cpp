...
bool odometryTaskRunning = false;

int odometryTask(void *params) {
  odometryTaskRunning = true;
  
  while(odometryTaskRunning) {
    updateOdometry();
    
    // Small delay to prevent CPU overload while still ensuring frequent updates
    wait(10, msec);
  }
