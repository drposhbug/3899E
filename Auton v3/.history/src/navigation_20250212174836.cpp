...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed){
    ...
    double currentHeading = getAdjustedHeading(); // Gets heading in CCW format
    // For CCW positive turns (positive means turn left)
    double headingError = ((targetHeading - currentHeading + 360) % 360);
    if (headingError > 180) {
        headingError -= 360;
    }
    ...
    //Convert % Speed input to voltage with max voltage of 12
    double maxSpeedVoltage = std::copysign(maxSpeed * 0.01 * 12, -headingError);  // Note the negative
    double minSpeedVoltage = std::copysign(minSpeed * 0.01 * 12, -headingError);  // Note the negative
    double launchVoltage = std::copysign(5, -headingError);  // Note the negative
...
while ((std::abs(headingError) > 5) && !crossed180) {
    double currentHeading = getAdjustedHeading(); // Gets heading in CCW format
    // Calculate shortest path error, ensuring counter-clockwise is positive
    double headingError = -getHeadingError360(currentHeading, targetHeading);  // Note: reversed order
    double currentDistanceInDegrees = headingError;
    
    if (std::round(currentHeading) * headingError < 0) {
        crossed180 = true;
    }
