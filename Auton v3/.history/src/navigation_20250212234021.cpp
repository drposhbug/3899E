...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed){
    ...
    double currentHeading = InertialSensor.heading(degrees);
    double rawHeadingError = targetHeading - currentHeading;
    double headingError = normHeading(rawHeadingError);
    ...
    //Convert % Speed input to voltage with max voltage of 12
    double maxSpeedVoltage = std::copysign(maxSpeed * 0.01 * 12, rawHeadingError);
    double minSpeedVoltage = std::copysign(minSpeed * 0.01 * 12, rawHeadingError);
    double launchVoltage = std::copysign(5, rawHeadingError); //3.5 is good
    double minLaunchSpeedVoltage = std::copysign(std::min(fabs(maxSpeedVoltage), fabs(launchVoltage)), rawHeadingError);
