...
// Calculate relative angle to turn (positive = clockwise, negative = counterclockwise)
    ...
    double turnAmount = fmod(atan2(deltaX, deltaY) * 180.0 / M_PI + 360.0, 360.0);  // Convert to 0-360 absolute heading
...
int odometryTask(void* params) {
   ...
   int counter = 0;
   double lastLeftEncoder = passiveEncoderLeft.position(vex::rotationUnits::deg);
   ...
   while(p->isRunning) {
       ...
       // Debug output
       counter++;
       double currentLeftEncoder = passiveEncoderLeft.position(vex::rotationUnits::deg);
       Brain.Screen.clearScreen();
       Brain.Screen.printAt(10, 120, "Task Counter: %d", counter);
       Brain.Screen.printAt(10, 140, "Left Encoder: %.2f", currentLeftEncoder);
       Brain.Screen.printAt(10, 160, "Encoder Change: %.2f", currentLeftEncoder - lastLeftEncoder);
