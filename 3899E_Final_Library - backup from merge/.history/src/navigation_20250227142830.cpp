...
void pivotTurnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed)
{
    ...
    while (std::abs(headingError) > 9) // Removed crossed180 check
    {
        ...
        */
        ...
        rightMotor[0].spin(forward, motorVoltageRight[0], voltageUnits::volt);  // Removed the negative sign
        leftMotor[1].stop(hold);
        rightMotor[1].spin(forward, motorVoltageRight[1], voltageUnits::volt);  // Removed the negative sign
        leftMotor[2].stop(brake);
        rightMotor[2].spin(forward, motorVoltageRight[2], voltageUnits::volt);  // Removed the negative sign
    } else { // Clockwise turn (negative error)
        // Right side stationary, left side moves
        leftMotor[0].spin(forward, -motorVoltageLeft[0], voltageUnits::volt);  // Added negative sign
        rightMotor[0].stop(brake);
        leftMotor[1].spin(forward, -motorVoltageLeft[1], voltageUnits::volt);  // Added negative sign
        rightMotor[1].stop(hold);
        leftMotor[2].spin(forward, -motorVoltageLeft[2], voltageUnits::volt);  // Added negative sign
