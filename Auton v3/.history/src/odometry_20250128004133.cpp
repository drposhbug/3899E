...
void calculatePathToTarget(double currentX, double currentY, 
    ...
    double targetAbsoluteHeading = atan2(targetY - currentY, targetX - currentX) * 180.0 / M_PI;
    
    // Get current robot heading
    double currentAbsoluteHeading = InertialSensor.heading();
    
    // Calculate relative heading by normalizing the difference
    // This means if our target heading matches our current heading, we get 0 (straight ahead)
    heading = normHeading(targetAbsoluteHeading - currentAbsoluteHeading);
    
    // Debug output
    Brain.Screen.printAt(10, 200, "AbsTgt=%.2f AbsCur=%.2f Rel=%.2f", 
                        targetAbsoluteHeading, currentAbsoluteHeading, heading);
}
