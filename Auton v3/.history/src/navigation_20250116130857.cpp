...
while ((std::fabs(currentDistanceInDegrees) <= std::fabs(targetDistanceInDegrees) - 5) && turnCompleted == false) {
   ...
   // double avgMotorRadianPerSecond =  totalMotorRadiansPerSecond / numberDriveMotor; // Calculate average motor radians per second
    ...
    // Get robot speed directly from encoders in RPM
    double leftEncoderRPM = passiveEncoderLeft.velocity(velocityUnits::rpm);
    double rightEncoderRPM = passiveEncoderRight.velocity(velocityUnits::rpm);
    robotRadiansPerSecond = (leftEncoderRPM + rightEncoderRPM) / 2.0;  // Average encoder RPM for robot speed
