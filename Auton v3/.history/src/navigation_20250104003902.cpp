...
void straight(double targetDistance, double maxSpeed, double targetHeading, double breakDistance, double kp_heading, double ki_heading, double kd_heading, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling, double minSpeed) { 
    ...
    double slipThreshold = 1.5; // the lower it is below 1, the slower and more controlled it will be
...
while (std::abs(currentDistance) <= std::abs(targetDistance) - 2) {
    ...
    passiveEncoderLeft = passiveEncoderLeft.velocity(vex::velocityUnits::rpm) * (wheelCircumferenceCM / encoderWheelCircumferenceCM);
    passiveEncoderRight = passiveEncoderRight.velocity(vex::velocityUnits::rpm) * (wheelCircumferenceCM / encoderWheelCircumferenceCM);
    double avgEncoderRPM = (passiveEncoderLeft + passiveEncoderRight)/2  * (wheelCircumferenceCM / encoderWheelCircumferenceCM);
...
if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && !accelCompleted) {
    ...
    double encoderRPMScaled = encoder.velocity(vex::velocityUnits::rpm) * (wheelCircumferenceCM / encoderWheelCircumferenceCM); 
        ...
        motorVoltageRight[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], avgEncoderRPM, accelFactorLaunch) - (adjustedHeadingCorrection * accelHeadingScaling);   
