...
void turnAbsolute(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed) {  
    // Reset completion flags
    bool crossed180 = false;
    bool decelCompleted = false;
    bool accelCompleted = false;
    ...
    // Get current heading and initial error
    double currentHeading = InertialSensor.heading(degrees);
    ...
    while ((std::abs(headingError) > 5) && !crossed180) {
        ...
        Brain.Screen.printAt(10, 100, "Head Error: %.2f", headingError);
        Brain.Screen.printAt(10, 120, "Curr Dist: %.2f", currentDistanceInDegrees);
        ...
        //Launch Phase
        if (fabs(currentDistanceInDegrees) < (fabs(targetHeading) - fabs(breakDistanceInDegrees)) && !accelCompleted && !decel) {
            for (int i = 0; i < 3; i++) {   
                motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i], leftMotorRPM[i], avgEncoderRPM, accelFactorLaunch);
                motorVoltageRight[i] = tractionControlRight[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], avgEncoderRPM, accelFactorLaunch);
            }  
        ...
        //Cruise Phase      
        } else if (fabs(currentDistanceInDegrees) < (fabs(targetHeading) - fabs(breakDistanceInDegrees)) && accelCompleted) {
            Brain.Screen.printAt(10, 20, "Cruise Phase");    
            for (int i = 0; i < 3; i++) {
                motorVoltageLeft[i] = maxSpeedVoltage;
                motorVoltageRight[i] = maxSpeedVoltage;
        ...
        // Decel Phase
        } else if (fabs(currentDistanceInDegrees) >= (fabs(targetHeading) - breakDistanceInDegrees) && decelCompleted == false) {
            decel = true; 
            Brain.Screen.printAt(10, 20, "Decel Phase");
