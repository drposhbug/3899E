...
if (Controller.ButtonY.pressing()) {
    ...
    // Move arm downward continuously instead of to a fixed position
    armMotor.setBrake(brakeType::hold);
    armMotor.spin(reverse, 30, velocityUnits::pct);  // Spin downward at 30% speed
    
    // Check for bumper while moving down
    if (armBumper.value() == 1) {
        armMotor.stop(brakeType::hold);  // Stop the motor
        armMotor.resetPosition();        // Reset position
...
} else if (armstat == ArmPosition::Load1 or armstat == ArmPosition::Load2) {
            // Move arm back to Ready position
                armMotor.setBrake(brakeType::hold);        
                armMotor.spinToPosition(Starting, rotationUnits::deg, 60, velocityUnits::pct, false);
                armstat = ArmPosition::Starting;
