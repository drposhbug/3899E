...
void turnLeft(double targetAngle, double breakDistance, double minSpeed, double maxSpeed) {
    // Just pass through - default behavior is what we want for left turns
    turnOdometry(targetAngle, breakDistance, minSpeed, maxSpeed);
}
...
void turnRight(double targetAngle, double breakDistance, double minSpeed, double maxSpeed) {
    double currentHeading = convertHeading(InertialSensor.heading(degrees), headingOffset);
    double headingError = normalizeHeading180(targetAngle - currentHeading);
    
    // Round to nearest degree
    headingError = round(headingError);
    
    // Only reverse sign if it rounds to exactly 180
    if (headingError == 180) {
