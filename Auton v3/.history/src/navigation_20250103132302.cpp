...
for (int i = 0; i < 3; i++) {   
    ...
    if (motor1.getBrake() == brakeType::brake)
...
c
        ...
        // Get only the brake mode from the ABS result
ABSReturn rightResult = ABSControllerRight[i].ABSSpeedReduction(
    motorVoltageRight[i], 
    rightMotorAngularRadians[i], 
    robotRadiansPerSecond
);

// Apply the brake mode directly
rightMotor[i].stop(rightResult.brakeMode);
