...
rightEncoderRollingAverage = rollingAverage(rightEncoderRPM,leftEncoderRollingAverage, 3);
...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed){
    ...
    double currentHeading = InertialSensor.heading(degrees);
    double headingError = normHeading(targetHeading - currentHeading);
    ...
    double launchVoltage = std::copysign(5, headingError); //3.5 is good
...
ABSController ABSControllerRight[3] = {
    ...
    while ((std::abs(headingError) > 5) && !crossed180) {
        currentHeading = InertialSensor.heading(degrees);
        headingError = normHeading(targetHeading - currentHeading);
...
void straightTemp(double targetDistance, 
    ...
    // PID and Heading Control
    double normTargetHeading = normHeading360(targetHeading);
    double avgMotorVoltage = 0;  // Used for phase transition checking
...
*/
        ...
        // Calculate the heading correction using the PID controller 
        double headingCorrection = headingPID.calculate(normTargetHeading, normHeading360(InertialSensor.heading()));
        double leftEncoderRPM = passiveEncoderLeft.velocity(vex::velocityUnits::rpm) * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
        double rightEncoderRPM = passiveEncoderRight.velocity(vex::velocityUnits::rpm) * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
        double avgEncoderRPM = (leftEncoderRPM + rightEncoderRPM)/2;
        //double adjustedHeadingCorrection = headingCorrection * avgEncoderRPM / absoluteMaxRPM; //dynamically reduce heading correction at slower speed based on percentage of max speed
...
void turn360(double turnDegrees, double breakDistanceInDegrees, double minSpeed, double maxSpeed) {  
    // Reset completion flags
    bool crossed180 = false;
    bool decelCompleted = false;
    bool accelCompleted = false;
    ...
    double normCurrentHeading = startingNormHeading;
    double currentDistanceInDegrees = 0;
    double targetDistanceInDegrees = normHeading360(turnDegrees);
...
while ((std::fabs(currentDistanceInDegrees) <= fabs(targetDistanceInDegrees) - 5) && !crossed180) {
    ...
    // Get current heading with offset conversion and normalize
    normCurrentHeading = normHeading360(convertHeading(InertialSensor.heading(degrees), headingOffset));
    currentDistanceInDegrees = normCurrentHeading - startingNormHeading;
...
ABSController ABSControllerRight[3] = {
    ...
    // Convert absolute target heading to rotation degrees
    double fullRotations = floor(InertialSensor.rotation() / 360.0);
    double baseRotation = fullRotations * 360.0;
    double targetRotation = baseRotation + targetHeading;                                               
...
void backward(double targetDistance, 
    ...
    Brain.Screen.clearScreen();
    Brain.Screen.printAt(10, 20, "Initial targetDist: %.2f", targetDistance);
    Brain.Screen.printAt(10, 40, "Initial heading: %.2f", targetHeading);
    ...
    Brain.Screen.printAt(10, 60, "maxSpeed after mod: %.2f", maxSpeed);
    ...
    // Target Speeds & Voltages
    // Target Speeds & Voltages
    double maxSpeedVoltage = -fabs(maxSpeed * 0.01 * absoluteMaxVoltage);  // Always negative
    double minSpeedVoltage = -fabs(minSpeed * 0.01 * absoluteMaxVoltage);  // Always negative
    double launchVoltage = -5;  // Always negative
    double minLaunchSpeedVoltage = -fabs(std::min(fabs(maxSpeedVoltage), fabs(launchVoltage)));
    static constexpr double VOLTAGE_TOLERANCE = 0.1;
    ...
    // Traction Control Parameters
    // slipThreshold: 0-1 range (0 = no slip allowed, 1 = full slip allowed, .15-.25 = optimal slip)
    double slipThresholdTraction = 0.75;
    double slipThresholdABS = 0.35;
    //double accelFactorLaunch = 1.4; //good starting launch acceleration factor
    double accelFactorLaunch = 1.15; //test, temporary
...
*/
        ...
        Brain.Screen.printAt(10, 120, "curr: %.2f targ: %.2f err: %.2f", 
            currentHeading, targetHeading, error);
...
*/
             ...
             void straightOdometryTest(double targetDistance, 
                double breakDistance, 
                double minSpeed, 
                double targetHeading, 
                double kp_heading,
        ...
        // Force maxSpeed and targetDistance to be negative since this is a backward function
        maxSpeed = -fabs(maxSpeed);  // Always make maxSpeed negative
        targetDistance = -fabs(targetDistance);  // Always make targetDistance negative
        ...
        // Since we're going backward, direction is always -1
        double headingDirection = -1.0;
