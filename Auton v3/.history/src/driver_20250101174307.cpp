...
*/
 ...
 //  }
    ...
    intakeMotor.spin(vex::directionType::fwd, 12, voltageUnits::volt); 
    } else {
...
*/
    ...
    if (Controller.ButtonY.pressing()) {
        if (!wasYPressed) { // Detect the press event
                ...
                } else if (armstat != ArmPosition::Starting && armstat != Load1) {
                    // Move arm to Alliance position
                    armMotor.setBrake(brakeType::hold);
                ...
                } else if (armstat == ArmPosition::Load1) { // Added condition
                    // Move arm back to Ready position
                    armMotor.setBrake(brakeType::hold);        
