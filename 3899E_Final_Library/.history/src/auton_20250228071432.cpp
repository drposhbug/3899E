...
const double BLUE_HUE_MAX = 230.0;  // Reduced from 240
   ...
   wait(300, msec);
    ...
    armMotor2.spinToPosition(Load1-75, // Add after intakeMotor.spin(reverse, 100, velocityUnits::pct);
        ...
        while (!ringDetected && timeoutCounter < maxTimeout) {
            ...
            if (ringDetected) {            
                // Optional: briefly pause the intake to stabilize ring position
                intakeMotor.stop();
                //wait(50, msec);
                // Resume intake at a stable power
                intakeMotor.spin(reverse, 75, velocityUnits::pct); // Lower power after capture
            }
            
            timeoutCounter++;
            wait(10, msec);
        }rotationUnits::deg, 100, velocityUnits::pct, false);     
    //wait(5000, msec);
