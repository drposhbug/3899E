...
while (std::abs(currentDistance) <= std::abs(targetDistance) - 2 && straightCompleted == false) {
...
if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && !straightCompleted && !accelCompleted) {
    ...
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        motorLeftRPM =  motor.velocity(vex::velocityUnits::rpm);
        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i],) + (adjustedHeadingCorrection * accelHeadingScaling);      // get slip voltage and Adjust for heading correction
        motorVoltageRight[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageRight[i]) - (adjustedHeadingCorrection * accelHeadingScaling);   
