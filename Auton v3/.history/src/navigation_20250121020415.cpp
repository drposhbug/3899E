...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    double motorVoltageLeft[3] = {minLaunchSpeedVoltage, minLaunchSpeedVoltage, minLaunchSpeedVoltage};  // Initialize all elements to minimum launch speed
    double motorVoltageRight[3] = {minLaunchSpeedVoltage, minLaunchSpeedVoltage, minLaunchSpeedVoltage};  // Initialize all elements to minimum launch speed
    vex::brakeType leftBrakeMode[3];   // Add here
vex::brakeType rightBrakeMode[3]; 
   ...
   // MotionPhase currentPhase = READY; 
    ...
    // Loop to continuously adjust motor power based on PID control 
while ((std::fabs(currentDistanceInDegrees) <= fabs(targetDistanceInDegrees) - 5) && !turnCompleted) {
...
if (fabs(currentDistanceInDegrees) < (fabs(targetDistanceInDegrees) - fabs(breakDistanceInDegrees)) && !turnCompleted && !accelCompleted && !decel) {
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
...
} else if (fabs(currentDistanceInDegrees) < (fabs(targetDistanceInDegrees) - fabs(breakDistanceInDegrees)) && accelCompleted == true) {
//break;   
...
//If declerating then go to ABS routine
} else if (abs(currentDistanceInDegrees) >= (abs(targetDistanceInDegrees) - breakDistanceInDegrees) && decelCompleted == false) {
//break;   
decel = true; 
Brain.Screen.printAt(10, 20, "Decel Phase");
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        vex::brakeType leadingPairMode = (leftBrakeMode[0] == brakeType::coast || rightBrakeMode[2] == brakeType::coast) ? 
                                        brakeType::coast : brakeType::brake;
        leftMotor[0].stop(leadingPairMode);
        rightMotor[2].stop(leadingPairMode);

        // Pair 2: Middle wheels (left[1] and right[1])
        vex::brakeType middlePairMode = (leftBrakeMode[1] == brakeType::coast || rightBrakeMode[1] == brakeType::coast) ? 
                                        brakeType::coast : brakeType::brake;
        leftMotor[1].stop(middlePairMode);
        rightMotor[1].stop(middlePairMode);

        // Pair 3: Trailing wheels (left[2] and right[0])
        vex::brakeType trailingPairMode = (leftBrakeMode[2] == brakeType::coast || rightBrakeMode[0] == brakeType::coast) ? 
                                        brakeType::coast : brakeType::brake;
