...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    // Reset completion flags at start of each turn
    boolen turnCompleted = false;
    decelCompleted = false;
    accelCompleted = false;
    ...
    while ((std::fabs(currentDistanceInDegrees) <= std::fabs(turnDegrees) - 5) && !turnCompleted) {
        // Get current heading and calculate how far we've turned
        normCurrentHeading = normHeading(InertialSensor.heading(degrees));
    ...
    //double avgMotorVoltage = 0;
    double launchVoltage = std::copysign(3, targetDistanceInDegrees); //3.5 is good
    ...
    double slipThresholdSpotTurn = 0.8; // the lower it is below 1, the slower and more controlled it will be
   ...
   // MotionPhase currentPhase = READY; 
    ...
    // Loop to continuously adjust motor power based on PID control 
while ((std::fabs(currentDistanceInDegrees) <= std::fabs(targetDistanceInDegrees) - 5) && turnCompleted == false) {
        // Update current heading
        ...
        // Debug prints during turn
        Brain.Screen.printAt(10, 100, "Curr Head: %.2f", currentNormHeading);
        Brain.Screen.printAt(10, 120, "Curr Dist: %.2f", currentDistanceInDegrees);
        Brain.Screen.printAt(10, 140, "Target Dist: %.2f", targetDistanceInDegrees);
        
        // Modified completion check - only use distance-based completion
        if (std::fabs(currentDistanceInDegrees) >= std::fabs(targetDistanceInDegrees) - 5) {
            turnCompleted = true;
            Brain.Screen.printAt(10, 160, "Turn Complete!");
        }
/*
...
// Get motor RPM with adjustment for each side
    ...
    // Get encoder RPM with circumference adjustment for each side
    double leftEncoderRPM = fabs(passiveEncoderLeft.velocity(velocityUnits::rpm)) * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
    double rightEncoderRPM = fabs(passiveEncoderRight.velocity(velocityUnits::rpm)) * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
...
if (fabs(currentDistanceInDegrees) < (fabs(targetDistanceInDegrees) - fabs(breakDistanceInDegrees)) && !turnCompleted && !accelCompleted) {
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i], leftMotorRPM[i], leftEncoderRPM, accelFactorLaunch);
        motorVoltageRight[i] = tractionControlRight[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], rightEncoderRPM, accelFactorLaunch);
...
} else if (std::fabs(currentDistanceInDegrees) < (std::fabs(targetDistanceInDegrees) - breakDistanceInDegrees) && accelCompleted == true) {
//break;   
...
decel = true; 
  ...
  Brain.Screen.printAt(10, 20, "Decel Phase");
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        //Left Side
        vex::brakeType leftBrakeMode = ABSControllerLeft[i].ABSSpeedReduction(leftMotorRPM[i], leftEncoderRPM);
        //vex::brakeType leftBrakeMode = ABSControllerLeft[i].ABSSpeedReduction(rightMotorAngularRadians[i], robotRadiansPerSecond); // Direct brake mode
        //leftMotor[i].setBrake(coast); // Set brake mode from ABSResult
        leftMotor[i].stop(leftBrakeMode);

        //Right Side
        vex::brakeType rightBrakeMode = ABSControllerRight[i].ABSSpeedReduction(rightMotorRPM[i], rightEncoderRPM);
...
//if (abs(robotRadiansPerSecond) <= abs(minRadiansPerSecond * (1 - percentSpeedLoss))) {
 averageMotorVoltage = (motorVoltageLeft[0] + motorVoltageRight[0] + motorVoltageLeft[1] + motorVoltageRight[1] + motorVoltageLeft[2] + motorVoltageRight[2]) / numberDriveMotor;
if (fabs(averageMotorVoltage) <= fabs(minSpeedVoltage)) {    
    decelCompleted = true;
