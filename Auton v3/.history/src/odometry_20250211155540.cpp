...
// Calculate relative angle to turn (positive = clockwise, negative = counterclockwise)
    ...
    // Get the current heading (already in CCW from getAdjustedHeading)
    double currentHeading = getAdjustedHeading();

    // Error calculation (both values now in CCW)
