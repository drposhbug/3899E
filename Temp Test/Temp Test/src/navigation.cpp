...
void smartMove(double distanceCM, double maxSpeed, int dir, double wallStalledTimeMs) {
    ...
    while (fabs(currentDistance) < fabs(distanceCM) && !wallDetected) {
        ...
        double leftEncoderRPM = fabs(passiveEncoderLeft.get_velocity() / 600.0);
        double rightEncoderRPM = fabs(passiveEncoderRight.get_velocity() / 600.0);
...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed, double exitTolerance) {
    ...
    while ((maxSpeedVoltage > 0 && currentHeading <= targetRotationHeading - exitTolerance) ||
        ...
        double leftEncoderRPM = fabs(passiveEncoderLeft.get_velocity() / 600.0) * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
        double rightEncoderRPM = fabs(passiveEncoderRight.get_velocity() / 600.0) * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
...
void straightOdometry(double targetDistance, double breakDistance, double targetHeading, double minSpeed,
    ...
    while (std::fabs(currentDistance) <= fabs(targetDistance) - 6) {
        ...
        double leftEncoderRPM = passiveEncoderLeft.get_velocity() / 600.0 * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
        double rightEncoderRPM = passiveEncoderRight.get_velocity() / 600.0 * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
...
void pivotTurnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed) {
    ...
    while (std::abs(headingError) > 9) {
        ...
        // Get motor RPM
        for (int i = 0; i < 3; i++) {
            leftMotorRPM[i] = fabs(leftMotor[i]->get_actual_velocity());
            rightMotorRPM[i] = fabs(rightMotor[i]->get_actual_velocity());
        }

        // Get encoder RPM - convert from deg/sec to RPM
        double leftEncoderRPM = fabs(passiveEncoderLeft.get_velocity() / 600.0) *
        ...
        if ((std::fabs(headingError) > fabs(breakDistanceInDegrees)) && !accelCompleted && !decel) {
            ...
            averageMotorVoltage = (fabs(motorVoltageLeft[0]) + fabs(motorVoltageRight[0]) + fabs(motorVoltageLeft[1]) + 
                                  fabs(motorVoltageRight[1]) + fabs(motorVoltageLeft[2]) + fabs(motorVoltageRight[2])) / numberDriveMotor;
        ...
        // Decel Phase
            ...
            double leftMotorRPMDecel = fabs(leftMotor[1]->get_actual_velocity());
            double rightMotorRPMDecel = fabs(rightMotor[1]->get_actual_velocity());
            double leftEncoderRPMDecel = fabs(passiveEncoderLeft.get_velocity() / 600.0) *
                                        (encoderWheelCircumferenceCM / wheelCircumferenceCM) * ENCODER_RADIUS_RATIO;
            double rightEncoderRPMDecel = fabs(passiveEncoderRight.get_velocity() / 600.0) *
                                         (encoderWheelCircumferenceCM / wheelCircumferenceCM) * ENCODER_RADIUS_RATIO;
        ...
        // Power motors - pivot logic
        if (!decel || decelCompleted) {
            if (motorVoltageLeft[0] > 0) {
                // Pivot around right side
                leftMotor[0]->move_voltage(motorVoltageLeft[0] * 1000);
                rightMotor[0]->move_voltage(0);
                leftMotor[1]->move_voltage(motorVoltageLeft[1] * 1000);
                rightMotor[1]->set_brake_mode(pros::E_MOTOR_BRAKE_HOLD);
                rightMotor[1]->move_voltage(0);
                leftMotor[2]->move_voltage(motorVoltageLeft[2] * 1000);
                rightMotor[2]->move_voltage(0);
            } else {
                // Pivot around left side
                leftMotor[0]->move_voltage(0);
                rightMotor[0]->move_voltage(fabs(motorVoltageRight[0]) * 1000);
                leftMotor[1]->set_brake_mode(pros::E_MOTOR_BRAKE_HOLD);
                leftMotor[1]->move_voltage(0);
                rightMotor[1]->move_voltage(fabs(motorVoltageRight[1]) * 1000);
                leftMotor[2]->move_voltage(0);
                rightMotor[2]->move_voltage(fabs(motorVoltageRight[2]) * 1000);
            }
        }
        pros::delay(10);
    }
    ...
    for (int i = 0; i < 3; i++) {
        leftMotor[i]->set_brake_mode(pros::E_MOTOR_BRAKE_BRAKE);
        rightMotor[2-i]->set_brake_mode(pros::E_MOTOR_BRAKE_BRAKE);
        leftMotor[i]->move_voltage(0);
        rightMotor[2-i]->move_voltage(0);
    }
}
...
void smartStraight(double targetDistance, double breakDistance, double targetHeading, double minSpeed,
    ...
    while (std::fabs(currentDistance) <= fabs(targetDistance) - 6.9 && !wallDetected) {
        ...
        // Decel Phase
            ...
            double leftEncoderRPMD = passiveEncoderLeft.get_velocity() / 600.0 * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
            double rightEncoderRPMD = passiveEncoderRight.get_velocity() / 600.0 * (encoderWheelCircumferenceCM / wheelCircumferenceCM);
