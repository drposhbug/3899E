        ...
        if (!decel == true || decelCompleted == true)
        {
            if (headingError > 0) { // Counter-clockwise turn (positive error)
                // Left side stationary, right side moves
                    leftMotor[1].stop(coast); // Left side stays stationary
                    rightMotor[1].spin(forward, -motorVoltageRight[0], voltageUnits::volt);
                    leftMotor[2].stop(hold); // Left side stays stationary
                    rightMotor[2].spin(forward, -motorVoltageRight[1], voltageUnits::volt);
                    leftMotor[3].stop(coast); // Left side stays stationary
                    rightMotor[3].spin(forward, -motorVoltageRight[2], voltageUnits::volt);
            
            } else { // Clockwise turn (negative error)
                // Right side stationary, left side moves
                    leftMotor[1].spin(forward, motorVoltageLeft[0], voltageUnits::volt);
                    rightMotor[1].stop(coast); // Right side stays stationary
                    leftMotor[2].spin(forward, motorVoltageLeft[1], voltageUnits::volt);
                    rightMotor[2].stop(hold); // Right side stays stationary
                    leftMotor[3].spin(forward, motorVoltageLeft[2], voltageUnits::volt);
                    rightMotor[3].stop(coast); // Right side stays stationary
                }
