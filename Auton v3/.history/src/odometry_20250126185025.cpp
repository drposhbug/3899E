...
// Calculate relative angle to turn (positive = clockwise, negative = counterclockwise)
    ...
    double turnAmount = -atan2(deltaY, deltaX) * 180.0 / M_PI;  // Negative because positive is clockwise

    // Debug prints
    Brain.Screen.printAt(10, 20, "deltaX: %.2f, deltaY: %.2f", deltaX, deltaY);
    Brain.Screen.printAt(10, 40, "turnAmount: %.2f", turnAmount);
    Brain.Screen.printAt(10, 60, "globalX: %.2f, globalY: %.2f", globalX, globalY);
