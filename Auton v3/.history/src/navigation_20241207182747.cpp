...
void pidStraightDistanceLaunchABS(double targetHeading, double targetDistance, double maxSpeed, double kp_heading, double ki_heading, double kd_heading, double kp_distance, double ki_distance, double kd_distance, double minSpeed, double breakDistance) { 
    ...
    double minSpeedVoltage = std::copysign(minSpeed * 0.01 * 12, targetDistance);
    double avgMotorVoltage = 0;
    double minLaunchSpeedVoltage = 12;
...
//Launch Phase
if ( std::abs(encoderAvgRPMScaled) < (maxDriveMotorRPM * (1- percentFrictionLoss))  &&  std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && accelCompleted == false) {
    ...
    //    Brain.Screen.printAt(10, 20, "Launch Phase");
     ...
     if (avgMotorVoltage >= maxSpeedVoltage){
        accelCompleted = true;
    }
