...
void pivotTurnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed)
{
    ...
    while (std::abs(headingError) > 9) // Removed crossed180 check
    {
        ...
        */
        ...
        if (!decel == true || decelCompleted == true)
        {
            Copyif (headingError > 0) { // Counter-clockwise turn (positive error)
                // Left side stationary, right side moves forward
                leftMotor[0].stop(brake); 
                rightMotor[0].spin(forward, -motorVoltageRight[0], voltageUnits::volt);  // Note the negative sign
                leftMotor[1].stop(hold); 
                rightMotor[1].spin(forward, -motorVoltageRight[1], voltageUnits::volt);  // Note the negative sign
                leftMotor[2].stop(brake); 
                rightMotor[2].spin(forward, -motorVoltageRight[2], voltageUnits::volt);  // Note the negative sign
