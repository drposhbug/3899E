...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    // Get normalized headings
    double startingNormHeading = normHeading(currentHeading);
    double normTargetHeading = normHeading(targetHeading);
    double normCurrentHeading = startingNormHeading;
    
    // Calculate the turn distance (this is what we'll track)
    double targetDistanceInDegrees = turnDegrees;
    
    // Debug prints at start of turn
    Brain.Screen.printAt(10, 40, "Start Head: %.2f", startingNormHeading);
    Brain.Screen.printAt(10, 60, "Target Head: %.2f", normTargetHeading);
    Brain.Screen.printAt(10, 80, "Turn Amount: %.2f", turnDegrees);
