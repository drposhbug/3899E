...
if ((motorRPM > 0 && encoderRPMScaled >= 0) || (motorRPM < 0 && encoderRPMScaled <= 0)) {// only check for slip if wheels and encoder are spining in same direction
    if (std::abs(motorRPM) > std::abs(encoderRPMScaled) * (slipThreshold)) {                      // Checking for slip plus slip threshold allowance
        motorPower = ((motorRPM / accelerationFactor) / maxRPM) * 100;         
    } else {
        motorPower = ((motorRPM * accelerationFactor) / maxRPM) * 100;      
    }
}
...
void pidStraightDistanceABS(double targetHeading, double targetDistance, double maxSpeed, double kp_heading, double ki_heading, double kd_heading, double kp_distance, double ki_distance, double kd_distance, double minSpeed, double breakDistance) { 
    ...
    double minDriveMotorRPM = (minSpeedVoltage/maxVoltage) * maxRPM;    
    //int stabilityCounter = 0;                      // Counts consecutive iterations within target zone
...
void pidStraightDistanceSlipABS(double targetHeading, double targetDistance, double maxSpeed, double kp_heading, double ki_heading, double kd_heading, double kp_distance, double ki_distance, double kd_distance, double minSpeed, double breakDistance) { 
    ...
    double minDriveMotorRPM = (minSpeedVoltage/maxVoltage) * maxRPM;
    double maxDriveMotorRPM = (maxSpeedVoltage/maxVoltage) * maxRPM; 
    double currentDistance = 0;
    double targetDriveVoltageLeft = 0; 
    double targetDriveVoltageRight = 0;
...
while ( std::abs(currentDistance) <=  std::abs(targetDistance) - 2) {
    ...
    double adjustedHeadingCorrection = headingCorrection * encoderAvgRPMScaled / maxRPM; //dynamically reduce heading correction at slower speed based on percentage of max speed
...
//Final Approach Phase    
        ...
        double adjustedHeadingCorrection = headingCorrection * encoderAvgRPMScaled / maxRPM * 2.0; //dynamically reduce heading correction at slower speed based on percentage of max speed
...
void pidStraightDistanceLaunchABS(double targetHeading, double targetDistance, double maxSpeed, double kp_heading, double ki_heading, double kd_heading, double accelHeadingScaling, double decelHeadingScaling, double minSpeed, double breakDistance) { 
    ...
    double minDriveMotorRPM = (minSpeed * .01) * maxRPM; //convert minspeed to percentage first
    double maxDriveMotorRPM = (maxSpeed * .01) * maxRPM; //convert maxspeed to percentage first
...
void spotTurnMP(double targetHeading, double maxSpeed, double minSpeed, double breakDistanceInDegrees, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling) { 
    ...
    double avgMotorVoltage = 0;
    double minLaunchSpeedVoltage = 5;
    double percentSpeedLoss = 0.15;
    double totalMotorRadiansPerSecond = 0.0;
    double minRadiansPerSecond = (minSpeed * .01) * maxRPM * (2 * M_PI / 60.0); //convert minspeed to percentage first
    double maxRadiansPerSecond = (maxSpeed * .01) * maxRPM * (2 * M_PI / 60.0); //convert maxspeed to percentage first
    ...
    double targetDriveVoltageLeft = 0; 
    double targetDriveVoltageRight = 0;
    double leftMotorRadiansPerSecond[3];
    double rightMotorRadiansPerSecond[3];
   ...
   // MotionPhase currentPhase = READY; 
    ...
    // Declare arrays of Slip Control instances for each wheel
    tractionControl tractionControlLeft[3] = {tractionControl(leftMotorRadiansPerSecond[0], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage),
                                              tractionControl(leftMotorRadiansPerSecond[1], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage),
                                              tractionControl(leftMotorRadiansPerSecond[2], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage)};
                                
    tractionControl tractionControlRight[3] = {tractionControl(rightMotorRadiansPerSecond[0], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage),
                                               tractionControl(leftMotorRadiansPerSecond[1], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage),
                                               tractionControl(leftMotorRadiansPerSecond[2], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage)};
...
antiLockBrake ABSRight[3] = {antiLockBrake(rightMotor[0], passiveEncoderRight, minSpeedVoltage, coast),
                             ...
                             antiLockBrake(rightMotor[2], passiveEncoderRight, minSpeedVoltage, coast)};
*/
ABSController ABSControllerLeft[3] = {ABSController(leftMotorRadiansPerSecond[0], robotRadiansPerSecond, minSpeedVoltage),
                  ABSController(leftMotorRadiansPerSecond[1], robotRadiansPerSecond, minSpeedVoltage),
                  ABSController(leftMotorRadiansPerSecond[2], robotRadiansPerSecond, minSpeedVoltage)};
                            
ABSController ABSControllerRight[3] = {ABSController(rightMotorRadiansPerSecond[0], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage),
                   ABSController(rightMotorRadiansPerSecond[1], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage),
                   ABSController(rightMotorRadiansPerSecond[2], robotRadiansPerSecond, minSpeedVoltage, maxSpeedVoltage)};
...
while (std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        leftMotorRadiansPerSecond[i] = leftMotors[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
        rightMotorRadiansPerSecond[i] = leftMotors[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
        
        //Add up all radians per second to prepare for calculating average motor radians per second
        totalMotorRadiansPerSecond += leftMotorRadiansPerSecond[i] + rightMotorRadiansPerSecond[i];
    }
    ...
    double robotDegreesPerSecond = InertialSensor.gyroRate(axisType::zaxis, velocityUnits::dps);
    robotRadiansPerSecond = robotDegreesPerSecond * (M_PI / 180.0);
...
if (std::abs(currentDistanceInDegrees) < (std::abs(targetDistanceInDegrees) - breakDistanceInDegrees) && accelCompleted == false) {
    ...
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i]);      // get slip voltage 
        motorVoltageRight[i] = tractionControlRight[i].tractionControlSpeed(motorVoltageRight[i]);   
 ...
 break;   
  Brain.Screen.printAt(10, 20, "Decel Phase");
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        //Right Side
        ABSResult rightResult = ABSControllerRight[i].ABSSpeedReduction(motorVoltageRight[i]); // Get ABSResult containing motor voltage and brake mode
        motorVoltageRight[i] = rightResult.motorVoltage; // Extract motor voltage from ABSResult
        if (motorVoltageRight[i] != 0) {
...
// Power Drive Motors        
        ...
        leftMotor[i].spin(forward, motorVoltageLeft[i], voltageUnits::volt);
        rightMotor[i].spin(reverse, motorVoltageRight[i], voltageUnits::volt); 
        }
...
tractionControl::tractionControl(double motorSpeed, double robotSpeed, double minSpeedVoltage, double maxSpeedVoltage)
    : motorSpeed(motorSpeed), robotSpeed(robotSpeed), minSpeedVoltage(minSpeedVoltage), maxSpeedVoltage(maxSpeedVoltage) {} // Initialized slipThreshold

    // adjustSpeed method implementation
    double tractionControl::tractionControlSpeed(double tractionMotorVoltage) { 

   // Brain.Screen.printAt(10, 100, "Current Speed: %.2f, Encoder Speed: %.2f", currentSpeed, encoderSpeed);

    Brain.Screen.printAt(10, 20, "Launch Phase");
    if (std::abs(motorSpeed) > std::abs(robotSpeed) * (slipThreshold)) {                      // Checking for slip plus slip threshold allowance
       tractionMotorVoltage = tractionMotorVoltage / accelerationFactor;    
