...
decel = true; 
  ...
  Brain.Screen.printAt(10, 20, "Decel Phase");
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        //Right Side
        vex::brakeType rightBrakeMode = ABSControllerRight[i].ABSSpeedReduction(rightMotorAngularRadians[i], robotRadiansPerSecond); // Direct brake mode
...
vex::brakeType ABSController::ABSSpeedReduction(double wheelSpeed, double robotSpeed) {
    if (std::abs(wheelSpeed) >= std::abs(robotSpeed) * ABSLockThreshold) {
        return vex::brake; // Prevent further locking
    } else {
        return vex::coast; // Apply brakes to slow down     
...
void straight(double targetDistance, double maxSpeed, double targetHeading, double breakDistance, double kp_heading, double ki_heading, double kd_heading, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling, double minSpeed) { 
    ...
    double minSpeedVoltage = std::copysign(minSpeed * 0.01 * 12, targetDistance);
    double avgMotorVoltage = 0;
    double launchVoltage = std::copysign(12, targetDistance); 
...
for (int i = 0; i < 3; i++) {   
    ...
    leftMotor[i].stop(leftBrakeMode);
     Brain.Screen.printAt(10, 140, "LT Brake Mode Value: %d", static_cast<int>(leftBrakeMode));
    ...
    Brain.Screen.printAt(10, 160, "RT Brake Mode Value: %d", static_cast<int>(rightBrakeMode));

    }
      
double avgMotorRPM = leftMotorRPM[0] + leftMotorRPM[1] + leftMotorRPM[2] + rightMotorRPM[0] + rightMotorRPM[1] + rightMotorRPM[2];

        Brain.Screen.printAt(10, 100, "Motor Speed: %.2f, Robot Speed: %.2f", avgMotorRPM,avgEncoderRPM ); 
