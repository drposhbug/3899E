...
void spotTurnMP(double turnDegrees, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    double encoderMotorScaleFactor = 1;//0.857143
    //double ABSLockThresholdSpotTurn = 0;
...
// Get motor RPM with adjustment for each side
    for (int i = 0; i < 3; i++) {
        ...
        leftMotorRPM[i] = fabs(leftMotor[i].velocity(velocityUnits::rpm) * encoderMotorScaleFactor);
        rightMotorRPM[i] = fabs(rightMotor[i].velocity(velocityUnits::rpm) * encoderMotorScaleFactor);
    }
...
if (fabs(currentDistanceInDegrees) < (fabs(targetDistanceInDegrees) - fabs(breakDistanceInDegrees)) && !crossed180 && !accelCompleted && !decel) {
  ...
  //  }
    ...
    if (std::fabs(averageMotorVoltage) >= std::fabs(maxSpeedVoltage)){
        ...
        Brain.Screen.printAt(10, 180, "RightVolt[0]: %.2f", motorVoltageRight[0]);
