...
/*
        ...
        double lowerVoltageLeading = std::min(motorVoltageLeft[0], motorVoltageRight[2]);
        motorVoltageLeft[0] = lowerVoltageLeading;
        motorVoltageRight[2] = lowerVoltageLeading;

        // Pair 2: Middle wheels
        double lowerVoltageMiddle = std::min(motorVoltageLeft[1], motorVoltageRight[1]);
        motorVoltageLeft[1] = lowerVoltageMiddle;
        motorVoltageRight[1] = lowerVoltageMiddle;

        // Pair 3: Trailing wheels
        double lowerVoltageTrailing = std::min(motorVoltageLeft[2], motorVoltageRight[0]);
        motorVoltageLeft[2] = lowerVoltageTrailing;
        motorVoltageRight[0] = lowerVoltageTrailing;    
*/        
...
//Launch Phase
if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && !straightCompleted && !accelCompleted) {
