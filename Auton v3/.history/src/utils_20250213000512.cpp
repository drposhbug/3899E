...
double normHeading(double heading) {
    ...
    // Instead of:
    // if ((heading + 180.0) > 0 && normHeading == -180) { normHeading = 180.0; }
    // Force a consistent result for ±180:
    if (std::fabs(normHeading) == 180.0) {
        normHeading = -180.0;  // or 180.0, whichever direction you want by default
    }
