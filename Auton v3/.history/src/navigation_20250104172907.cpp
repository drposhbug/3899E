...
decel = true; 
  ...
  Brain.Screen.printAt(10, 20, "Decel Phase");
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        //Left Side
        ABSReturn leftResult = ABSControllerLeft[i].ABSSpeedReduction(leftMotorAngularRadians[i], robotRadiansPerSecond); // Get ABSResult containing motor voltage and brake mode
        ...
        //Right Side
        ABSReturn rightResult = ABSControllerRight[i].ABSSpeedReduction(rightMotorAngularRadians[i], robotRadiansPerSecond); // Get ABSResult containing motor voltage and brake mode
...
ABSReturn ABSController::ABSSpeedReduction(double wheelSpeed, double robotSpeed) {
    ...
    private:
        static constexpr double ABSLockThreshold = 0.7; // Class-wide constant
    };
...
void straight(double targetDistance, double maxSpeed, double targetHeading, double breakDistance, double kp_heading, double ki_heading, double kd_heading, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling, double minSpeed) { 
    ...
    double ABSLockThreshold = 20;
    double accelFactorLaunch = 14;
    bool decel = false;
    bool decelCompleted = false;
    bool accelCompleted = false;
...
for (int i = 0; i < 3; i++) {   
    ...
    //Left Side
    ABSReturn leftResult = ABSControllerLeft[i].ABSSpeedReduction(motorVoltageLeft[i], leftMotorRPM[i], leftEncoderRPM); // Get ABSResult containing motor voltage and brake mode
     motorVoltageLeft[i] = leftResult.motorVoltage ; 
    ...
    //Right Side
    ABSReturn rightResult = ABSControllerRight[i].ABSSpeedReduction(motorVoltageRight[i], rightMotorRPM[i], rightEncoderRPM); // Get ABSResult containing motor voltage and brake mode
    motorVoltageRight[i] = rightResult.motorVoltage ; // Extract motor voltage from ABSResult
