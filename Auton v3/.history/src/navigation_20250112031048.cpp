...
if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && !accelCompleted && !decel) {
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        motorVoltageRight[i] = tractionControlRight[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], rightEncoderRPM, accelFactorLaunch) - (adjustedHeadingCorrection * accelHeadingScaling * headingDirection);     
    }  
