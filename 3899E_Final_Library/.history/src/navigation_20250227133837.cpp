            ...
            if (!decel == true || decelCompleted == true)
{
    if (headingError > 0) { // Counter-clockwise turn (positive error)
        ...
        rightMotor[0].spin(reverse, fabs(motorVoltageRight[0]), voltageUnits::volt);  // Changed direction and force positive
        leftMotor[1].stop(hold);
        rightMotor[1].spin(reverse, fabs(motorVoltageRight[1]), voltageUnits::volt);  // Changed direction and force positive
        leftMotor[2].stop(brake);
        rightMotor[2].spin(reverse, fabs(motorVoltageRight[2]), voltageUnits::volt);  // Changed direction and force positive
    } else { // Clockwise turn (negative error)
        // Right side stationary, left side moves
        leftMotor[0].spin(reverse, fabs(motorVoltageLeft[0]), voltageUnits::volt);  // Changed direction and force positive
        rightMotor[0].stop(brake);
        leftMotor[1].spin(reverse, fabs(motorVoltageLeft[1]), voltageUnits::volt);  // Changed direction and force positive
        rightMotor[1].stop(hold);
        leftMotor[2].spin(reverse, fabs(motorVoltageLeft[2]), voltageUnits::volt);  // Changed direction and force positive
