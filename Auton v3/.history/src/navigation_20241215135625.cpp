...
while (std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        motorRadiansPerSecondLeft[i] = leftMotor[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
        motorRadiansPerSecondRight[i] = rightMotor[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
...
if (std::abs(currentDistanceInDegrees) < (std::abs(targetDistanceInDegrees) - breakDistanceInDegrees) && accelCompleted == false) {
    ...
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        motorRadiansPerSecondLeft[i] = InertialSensor.gyroRate(axisType::zaxis, velocityUnits::dps) * (M_PI / 180.0);
        motorRadiansPerSecondRight[i] = InertialSensor.gyroRate(axisType::zaxis, velocityUnits::dps) * (M_PI / 180.0);

        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i], motorRadiansPerSecondLeft[i], robotRadiansPerSecond);      // get slip voltage 
        motorVoltageRight[i] = tractionControlRight[i].tractionControlSpeed(motorVoltageRight[i], motorRadiansPerSecondRight[i], robotRadiansPerSecond);   
