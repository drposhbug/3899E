...
void calculatePathToTarget(double currentX, double currentY, 
    ...
    // Calculate absolute heading to target
    double absoluteHeading = atan2(targetY - currentY, targetX - currentX) * 180.0 / M_PI;
    
    // Get current robot heading
    double currentRobotHeading = InertialSensor.heading();
    
    // Calculate relative heading (how much we need to turn from current heading)
    // Since straight ahead should be 0, we subtract the robot's current heading
    heading = normHeading(absoluteHeading - currentRobotHeading);
    
    // Debug output
    Brain.Screen.printAt(10, 200, "AbsHead=%.2f CurHead=%.2f RelHead=%.2f", 
                        absoluteHeading, currentRobotHeading, heading);
    Brain.Screen.printAt(10, 220, "dx=%.2f dy=%.2f dist=%.2f", 
                        targetX - currentX, targetY - currentY, distance);
}
