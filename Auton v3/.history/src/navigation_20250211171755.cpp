...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed){
    ...
    // Compute error as currentHeading - continuousTarget so that a positive target (e.g. 117)
    // yields a negative error when currentHeading is 0.
    double headingError = normHeading(continuousTarget - currentHeading);
...
while (std::abs(headingError) > 5) {  // Remove crossed180 check
    ...
    // Use the same subtraction order here.
    headingError = normHeading(continuousTarget - currentHeading);
...
//Launch Phase
//if (std::fabs(currentDistanceInDegrees) < (std::fabs(targetDistanceInDegrees) - breakDistanceInDegrees) && !accelCompleted && !turnCompleted) {
if ((fabs(headingError) > (fabs(targetHeading) - fabs(breakDistanceInDegrees))) && !accelCompleted && !decel){
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i], leftMotorRPM[i], avgEncoderRPM, accelFactorLaunch);
        motorVoltageRight[i] = tractionControlRight[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], avgEncoderRPM, accelFactorLaunch);
...
} else if ((fabs(headingError) > (fabs(targetHeading) - fabs(breakDistanceInDegrees))) && accelCompleted){
//break;   
...
//If declerating then go to ABS routine
} else if ((fabs(headingError) <= (fabs(targetHeading) - fabs(breakDistanceInDegrees))) && decelCompleted == false) {
decel = true; 
Brain.Screen.printAt(10, 20, "Decel Phase");
