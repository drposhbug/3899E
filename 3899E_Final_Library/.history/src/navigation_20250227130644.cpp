...
void pivotTurnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed)
{
    ...
    double maxSpeedVoltage = std::copysign(maxSpeed * 0.01 * 12, headingError);
    double minSpeedVoltage = std::copysign(minSpeed * 0.01 * 12, headingError);
    double launchVoltage = std::copysign(6, headingError);
    double minLaunchSpeedVoltage = std::copysign(std::min(fabs(maxSpeedVoltage), fabs(launchVoltage)), headingError);
    ...
    // Loop to continuously adjust motor power based on PID control
    while (std::abs(headingError) > 4) // Removed crossed180 check
    {
        currentHeading = convertHeading(InertialSensor.heading(degrees), headingOffset);
        headingError = normalizeHeading180(targetHeading - currentHeading);
        currentDistanceInDegrees = headingError;
        ...
        */
        ...
        else if ((std::abs(headingError) <= fabs(breakDistanceInDegrees)) && decelCompleted == false)
        {
            ...
            // Pair 2: Middle wheels
            if (headingError > 0) { // Counter-clockwise turn (positive error)
                // Left side stationary, right side moves
                    leftMotor[1].stop(hold); // Left side stays stationary
                                
            } else { // Clockwise turn (negative error)
                // Right side stationary, left side moves
                    rightMotor[1].stop(hold); // Right side stays stationary
                }
        ...
        else if (decelCompleted == true)
{
    ...
    // Set motor voltages to minimum speed for final approach
    for (int i = 0; i < 3; i++)
    {
        // Set motor voltage to minimum speed for controlled approach
        motorVoltageLeft[i] = minSpeedVoltage;
        motorVoltageRight[i] = minSpeedVoltage;
    }
    ...
    Brain.Screen.printAt(10, 20, "Approach Phase");
        ...
        {
            if (headingError > 0) { // Counter-clockwise turn (positive error)
                // Left side stationary, right side moves forward
                leftMotor[0].stop(brake); // Left side stays stationary
                rightMotor[0].spin(forward, motorVoltageRight[0], voltageUnits::volt);  // Removed negative sign
                leftMotor[1].stop(hold); // Left side stays stationary
                rightMotor[1].spin(forward, motorVoltageRight[1], voltageUnits::volt);  // Removed negative sign
                leftMotor[2].stop(brake); // Left side stays stationary
                rightMotor[2].spin(forward, motorVoltageRight[2], voltageUnits::volt);  // Removed negative sign
            } else { // Clockwise turn (negative error)
                // Right side stationary, left side moves
                leftMotor[0].spin(forward, motorVoltageLeft[0], voltageUnits::volt);
                rightMotor[0].stop(brake); // Right side stays stationary
                ...
                rightMotor[2].stop(brake); // Right side stays stationary
            }
        }
