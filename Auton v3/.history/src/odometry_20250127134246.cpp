...
void updateOdometry() {
    ...
    double leftDist = (deltaLeft * encoderWheelCircumferenceCM) / 360.0;
    double rightDist = (deltaRight * encoderWheelCircumferenceCM) / 360.0;
    double avgDeltaDistance = (leftDist + rightDist) / 2.0;
...
if (currentState == TURNING) {
    ...
    if (fabs(deltaHeadingRad) > 0.001) {
        // Calculate true arc radii using encoder offsets
        double leftRadius = sqrt(pow(TRACK_WIDTH/2.0, 2) + pow(LEFT_ENCODER_OFFSET_Y, 2));
        double rightRadius = sqrt(pow(TRACK_WIDTH/2.0, 2) + pow(RIGHT_ENCODER_OFFSET_Y, 2));
        
        // Calculate actual distances traveled by each encoder
        double leftArcDist = leftDist * (leftRadius / (TRACK_WIDTH/2.0));
        double rightArcDist = rightDist * (rightRadius / (TRACK_WIDTH/2.0));
        
        // Calculate average radius for position update
        double radius = (leftArcDist + rightArcDist) / (2.0 * deltaHeadingRad);
        
        // Calculate X,Y position changes based on arc motion
