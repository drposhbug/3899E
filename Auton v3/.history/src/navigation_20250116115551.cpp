...
vex::brakeType ABSController::ABSSpeedReduction(double wheelSpeed, double robotSpeed) {
    ...
    // Calculate slip ratio similar to traction control
    double slipRatio = fabs((wheelSpeed - robotSpeed) / wheelSpeed);
    
    // If slip exceeds threshold, release brakes to prevent lock
    if (slipRatio > ABSLockThreshold) {
        return vex::coast;  // Release brakes to prevent lock
    } else {
        return vex::brake;  // Apply brakes - grip is good
    }
}
...
*/
    ...
    //}       
        ...
        // Stop all motors at end of routine after approach
        for (int i = 0; i < 3; i++) {
            leftMotor[i].stop(coast); //changed to coast temporarilly to test ABS.  Need to chnage back to Brake.
