...
ABSReturn ABSController::ABSSpeedReduction(double ABSMotorVoltage) {
    ABSReturn return;  // Create ABSResult instance to store motor voltage and brake mode

    if (ABSMotorVoltage <= minSpeedVoltage) {
        // Wheel has reduced speed to the target or slower
        return.motorVoltage = minSpeedVoltage;  // Set motor voltage to min Speed
        return.brakeMode = vex::brakeType::coast;  // Prevent locking
        Brain.Screen.printAt(60, 20, "coast");
    } else {
        // Check wheel locking condition
        if (std::abs(motorSpeed) >= std::abs(robotSpeed) * ABSLockThreshold) {
            // Wheels are locking
            return.motorVoltage = 0;  // Set motor voltage to 0
            return.brakeMode = vex::brakeType::coast;  // Prevent further locking
            Brain.Screen.printAt(60, 40, "coast (lock)");
        } else {
            // Wheels are not locking but need braking
            return.motorVoltage = 0;  // Set motor voltage to 0
            return.brakeMode = vex::brakeType::brake;  // Apply brakes to slow down
            Brain.Screen.printAt(60, 60, "brake");
        }
    }

    return return;  // Return ABSResult containing motor voltage and brake mode
