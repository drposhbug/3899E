...
void turnOdometry(double targetHeading, double breakDistanceInDegrees, double minSpeed, double maxSpeed)
{
    ...
    Brain.Screen.printAt(10, 60, "Curr Heading: %.2f", 360-convertEuclideanToVEX(currentHeading));

    // Convert % Speed input to voltage with max voltage of 12
    double maxSpeedVoltage = std::copysign(maxSpeed * 0.01 * 12, (headingError*maxSpeed));
    double minSpeedVoltage = std::copysign(minSpeed * 0.01 * 12, (headingError*maxSpeed));
    double launchVoltage = std::copysign(6, (headingError*maxSpeed));
    double minLaunchSpeedVoltage = std::copysign(std::min(fabs(maxSpeedVoltage), fabs(launchVoltage)), (headingError*maxSpeed));
    ...
    // Loop to continuously adjust motor power based on PID control
    while (std::abs(headingError) > 5) // Removed crossed180 check
    {
        ...
        Brain.Screen.printAt(10, 100, "Curr Head: %.2f", 360-convertEuclideanToVEX(currentHeading));
        Brain.Screen.printAt(10, 120, "Curr Dist: %.2f", currentDistanceInDegrees);
