...
while (std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        double vLeft[i] = leftMotor[i].velocity(velocityUnits::rpm) * (wheelCircumferenceCM / 60.0);
        double vRight = rightMotor[i].velocity(velocityUnits::rpm) * (wheelCircumferenceCM / 60.0);
