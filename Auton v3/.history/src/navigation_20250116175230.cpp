...
void spotTurnMP(double targetHeading, double maxSpeed, double minSpeed, double breakDistanceInDegrees) { 
    ...
    //double avgMotorVoltage = 0;
    double launchVoltage = std::copysign(1.5, targetDistanceInDegrees); //3.5 is good
    double minLaunchSpeedVoltage = std::copysign(std::min(fabs(maxSpeedVoltage), fabs(launchVoltage)), targetHeading);
    double percentSpeedLoss = 0.2;
    double slipThresholdSpotTurn = 0; // the lower it is below 1, the slower and more controlled it will be
   ...
   // MotionPhase currentPhase = READY; 
    ...
    // Loop to continuously adjust motor power based on PID control 
while ((std::fabs(currentDistanceInDegrees) <= std::fabs(normTargetHeading) - 5) && turnCompleted == false) {
...
if (fabs(currentDistanceInDegrees) < (fabs(normTargetHeading) - fabs(breakDistanceInDegrees)) && !turnCompleted && !accelCompleted) {
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
  ...
  //  }
    ...
    if (std::fabs(averageMotorVoltage) >= std::fabs(maxSpeedVoltage)){
        accelCompleted = true;
        Brain.Screen.printAt(10, 40, "Accel Completed");
    }

  
        Brain.Screen.printAt(10, 20, "Launch Phase");
        //Brain.Screen.printAt(10, 180, "Current Heading: %.2f", currentNormHeading);
        Brain.Screen.printAt(10, 200, "Target Distance: %.2f", normTargetHeading);
...
} else if (std::fabs(currentDistanceInDegrees) < (std::fabs(normTargetHeading) - breakDistanceInDegrees) && accelCompleted == true) {
//break;   
...
//If declerating then go to ABS routine
} else if (std::abs(currentDistanceInDegrees) >= (std::abs(normTargetHeading) - breakDistanceInDegrees) && decelCompleted == false) {
break;   
decel = true; 
