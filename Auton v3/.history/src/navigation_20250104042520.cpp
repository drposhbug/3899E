...
void straight(double targetDistance, double maxSpeed, double targetHeading, double breakDistance, double kp_heading, double ki_heading, double kd_heading, double accelHeadingScaling, double decelHeadingScaling, double approachHeadingScaling, double minSpeed) { 
    ...
    double slipThreshold = 1.2; // the lower it is below 1, the slower and more controlled it will be
    double ABSLockThreshold = 0.7;
    double accelFactorLaunch = 1.00;
    bool decel = false;
    bool decelCompleted = false;
    bool accelCompleted = false;
...
while (std::abs(currentDistance) <= std::abs(targetDistance) - 2) {
    ...
    // Calculate the heading correction using the PID controller 
    double headingCorrection = headingPID.calculate(normalizeTargetHeading, normHeading(InertialSensor.heading()));
...
if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && !accelCompleted) {
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i], leftMotorRPM[i], leftEncoderRPM, accelFactorLaunch) + (adjustedHeadingCorrection * accelHeadingScaling);      // get slip voltage and Adjust for heading correction
        motorVoltageRight[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], rightEncoderRPM, accelFactorLaunch) - (adjustedHeadingCorrection * accelHeadingScaling);     
    }  
...
for (int i = 0; i < 3; i++) {   
    ...
    //Left Side
    ABSReturn leftResult = ABSControllerLeft[i].ABSSpeedReduction(motorVoltageLeft[i], leftMotorRPM[i], avgEncoderRPM); // Get ABSResult containing motor voltage and brake mode
    ...
    //Right Side
    ABSReturn rightResult = ABSControllerRight[i].ABSSpeedReduction(motorVoltageRight[i], rightMotorRPM[i], avgEncoderRPM); // Get ABSResult containing motor voltage and brake mode
    motorVoltageRight[i] = rightResult.motorVoltage; // Extract motor voltage from ABSResult
