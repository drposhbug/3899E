...
while (std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        double leftMOtorCMPerSecond[i] = leftMotor[i].velocity(velocityUnits::rpm) * (wheelCircumferenceCM / 60.0);
        double rightMOtorCMPerSecond[i] = rightMotor[i].velocity(velocityUnits::rpm) * (wheelCircumferenceCM / 60.0);

        double leftMotorAngularRadian[i] = (2 * fabs(leftMOtorCMPerSecond[i])) / trackWidth;
        double rightMotorAngularRadians[i] = (2 * fabs(rightMOtorCMPerSecond[i])) / trackWidth;
