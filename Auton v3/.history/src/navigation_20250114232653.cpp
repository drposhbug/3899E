...
if (std::fabs(currentDistanceInDegrees) < (std::fabs(targetDistanceInDegrees) - breakDistanceInDegrees) && !turnCompleted && !accelCompleted) {
    for (int i = 0; i < 3; i++) {   
        // Use leftLaunchControl if minLaunchPower threshold is met for the left side
...
void straight(double targetDistance, 
    ...
    double launchVoltage = std::copysign(11.89, targetDistance);
    ...
    while (std::fabs(currentDistance) <= std::fabs(targetDistance) - 2) {
    ...
    //Launch Phase
    if (std::fabs(currentDistance) < (std::fabs(targetDistance) - breakDistance) && !accelCompleted && !decel) {
        ...
        for (int i = 0; i < 3; i++) {   
            ...
            double maxMotorRPM = std::max(maxMotorRPM, rightMotorRPM[i]);
            double maxEncoderRPM = std::max(maxEncoderRPM, fabs(avgEncoderRPM));

           // Brain.Screen.setCursor(11,1);
            //Brain.Screen.print("MaxWS/RS: %.3f/%.3f", maxMotorRPM, maxEncoderRPM);
double maxRPM = std::max(maxRPM, leftMotor[0].velocity(velocityUnits::rpm));
double maxPctRPM = std::max(maxPctRPM, leftMotor[0].velocity(velocityUnits::pct) * 6);
double maxM1 = std::max(maxM1, leftMotor[0].velocity(velocityUnits::rpm));
double maxM2 = std::max(maxM2, leftMotor[1].velocity(velocityUnits::rpm));
double maxM3 = std::max(maxM3, leftMotor[2].velocity(velocityUnits::rpm));

Brain.Screen.setCursor(11,1);
Brain.Screen.print("MaxRPM/PctRPM: %.1f/%.1f", maxRPM, maxPctRPM);
Brain.Screen.setCursor(12,1);
Brain.Screen.print("MaxM1/M2/M3: %.1f/%.1f/%.1f", maxM1, maxM2, maxM3);
        }  

        if (fabs(avgMotorVoltage) >= fabs(maxSpeedVoltage - VOLTAGE_TOLERANCE)){
            
            // Record acceleration time when complete
            accelTime = accelTimer.time(vex::timeUnits::msec)/1000.0;  // Convert to seconds
                    
            // Display acceleration time
            Brain.Screen.setCursor(10,1);
            Brain.Screen.print("Accel Time: %.3f s", accelTime);
    ...
    //Cruise Phase      
    } else if (std::fabs(currentDistance) < (std::fabs(targetDistance) - breakDistance) && accelCompleted == true) {
    break;  
        Brain.Screen.printAt(10, 20, "Cruise Phase");    // Else condition - specify actions here if the if condition is not met
    ...
    // Decel Phase
    //If declerating then go to ABS routine
    } else if (std::fabs(currentDistance) >= (std::fabs(targetDistance) - breakDistance) && decelCompleted == false) {  
    break;   
