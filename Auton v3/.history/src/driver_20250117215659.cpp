...
if (Controller.ButtonY.pressing()) {
    ...
    armMotor.spin(reverse, 30, velocityUnits::pct);  // Spin downward until bumper hit
...
*/
   ...
   // Reset motor encoder
    ...
    // Bumper Button 
        if (armBumper.value() == 0) {
    if (!wasBumperPressed) {  // Only execute once when first pressed
    ...
    wasBumperPressed = false;  // Reset when bumper released
}
