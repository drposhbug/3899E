...
while (std::abs(currentDistanceInDegrees) <= std::abs(targetDistanceInDegrees) - 2) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        leftMotorRadiansPerSecond[i] = leftMotor[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
        rightMotorRadiansPerSecond[i] = rightMotor[i].velocity(velocityUnits::rpm) * (2 * M_PI / 60.0);
...
tractionControl::tractionControl(double motorSpeed, double robotSpeed, double minSpeedVoltage, double maxSpeedVoltage)
   ...
   Brain.Screen.printAt(10, 100, "Motor Speed: %.2f, Encoder Speed: %.2f", motorSpeed,robotSpeed);
