...
while (std::abs(currentDistance) <= std::abs(targetDistance) - 2) {
    ...
    double leftEncoderPRM = passiveEncoderLeft.velocity(vex::velocityUnits::rpm);
    double rightEncoderRPM = passiveEncoderRight.velocity(vex::velocityUnits::rpm);
    double avgEncoderRPM = (leftEncoderPRM + rightEncoderRPM)/2  * (wheelCircumferenceCM / encoderWheelCircumferenceCM);
