...
//If declerating then go to ABS routine
  ...
  Brain.Screen.printAt(10, 20, "Decel Phase");
    ...
    for (int i = 0; i < 3; i++) {   
        ...
        if (motorVoltageLeft[i] != 0) {
            motorVoltageLeft[i] += (adjustedHeadingCorrection * 0.5); // Add heading correction only if voltage is not zero
        }
        ...
        if (motorVoltageRight[i] != 0) {
            motorVoltageRight[i] -= (adjustedHeadingCorrection * 0.5); // Subtract heading correction only if voltage is not zero
        }
