...
void calculatePathToTarget(double currentX, double currentY, 
    ...
    double absoluteTargetHeading = -atan2(deltaX, deltaY) * 180.0 / M_PI;
    
    // Calculate relative heading by subtracting current heading
    // and normalize the result
    heading = normHeading(absoluteTargetHeading - globalHeading);

    // Debug output
Brain.Screen.clearScreen();  // Clear the screen first
    Brain.Screen.printAt(10, 200, "AbsTgt=%.2f AbsCur=%.2f Rel=%.2f", 
                        absoluteTargetHeading, globalHeading, heading);
}
