...
void straight(double targetDistance, 
    ...
    if (std::abs(currentDistance) < (std::abs(targetDistance) - breakDistance) && !accelCompleted && !decel) {
        ...
        for (int i = 0; i < 3; i++) {   
            ...
            motorVoltageLeft[i] = tractionControlLeft[i].tractionControlSpeed(motorVoltageLeft[i], leftMotorRPM[i], avgEncoderRPM, accelFactorLaunch) + (headingCorrection * accelHeadingScaling * headingDirection);      // get slip voltage and Adjust for heading correction
            motorVoltageRight[i] = tractionControlRight[i].tractionControlSpeed(motorVoltageRight[i], rightMotorRPM[i], avgEncoderRPM, accelFactorLaunch) - (headingCorrection * accelHeadingScaling * headingDirection);   
