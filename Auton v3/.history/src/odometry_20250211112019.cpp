...
// Add this helper function to get adjusted heading
...
void leftToPoint(double targetX, double targetY,
                 ...
                 double minSpeed, double maxSpeed)
{
    // Set state to TURNING and start timeout timer.
    currentState = TURNING;
    double startTime = Brain.Timer.time(msec);
    const double TIMEOUT = 3000; // 3 seconds max
    updateOdometry();

    // Calculate the target heading from the current position.
    double deltaX = targetX - globalX;
    double deltaY = targetY - globalY;
    double computedTarget = fmod(atan2(deltaY, deltaX) * 180.0 / M_PI + 360.0, 360.0);

    // Get the current heading.
    double currentHeading = InertialSensor.heading(degrees);

    // Compute the minimal angular error (in [-180, 180) degrees).
    double error = normHeading(computedTarget - currentHeading);

    // If the error is positive (meaning the minimal turn is right) and we want a left turn,
    // force a left turn by subtracting 360° from the error.
    if (error > 0) {
        error = error - 360.0;  // Now error is negative, forcing a left turn.
    }

    // Calculate the forced target heading from the current heading and the forced error.
    double forcedTarget = currentHeading + error;

    // Now perform the turn using the forced left-turn target.
    turnOdometry(forcedTarget, breakDistanceInDegrees, minSpeed, maxSpeed);
