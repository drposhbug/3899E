...
void resetOdometry() {
    ...
    prevLeftEncoder = passiveEncoderLeft.position(rotationUnits::deg);
    prevRightEncoder = passiveEncoderRight.position(rotationUnits::deg);
    prevXEncoder = passiveEncoderX.position(rotationUnits::deg);
...
void updateOdometry() {
    ...
    double leftEncoder = passiveEncoderLeft.position(rotationUnits::deg);   // Current left encoder value in degrees
    double rightEncoder = passiveEncoderRight.position(rotationUnits::deg); // Current right encoder value in degrees
    double xEncoder = xEncoderEnabled ? passiveEncoderX.position(rotationUnits::deg) : prevXEncoder; // Read from X encoder if enabled
